...
type Props = { title: string; unit: string; data: TrendPoint[]; formatValue: (value: number) => string; domain?: TrendDomain; className?: string };
...
export const deriveTrendDomain = (values: readonly number[]): TrendDomain => {
  const valid = values.filter(value => Number.isFinite(value));
  if (!valid.length) return { min: -1, max: 1, ticks: [1, 0, -1] };
...
export const deriveSharedTrendDomain = (series: ReadonlyArray<ReadonlyArray<number>>) => deriveTrendDomain(series.flat());
...
export const formatTrendAxisTick = (value: number, formatValue: (value: number) => string) => formatValue(value).replace(/\s*(?:萬元|萬|元)\s*$/, '');
...
export default function TrendChart({ title, unit, data, formatValue, domain, className = '' }: Props) {
  ...
  const axis = domain ?? deriveTrendDomain(valid.map(point => point.value));
