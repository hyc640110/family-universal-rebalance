...
export default function ImportCenter({ accounts, transactions, sessions, presets, onCommit, onRollback, onPresets }: ImportCenterProps) {
  ...
  return <div className="financial-account-list import-center">
    ...
    {preview.length > 0 && <><div className="import-preview">{preview.slice(0, 50).map(row => { const suggestion = row.categorySuggestion?.kind === 'suggestion' ? row.categorySuggestion : undefined; return <div className={row.error ? 'warning-message' : 'note'} key={row.rowNumber}><label><input type="checkbox" checked={row.selected} disabled={Boolean(row.error)} onChange={event => { const checked = event.currentTarget.checked; setPreview(current => current.map(item => item.rowNumber === row.rowNumber ? { ...item, selected: checked } : item)); }} /> 第 {row.rowNumber} 列｜{row.type ? transactionTypeLabel(row.type) : '方向未確認'}｜{row.description || '—'}｜{row.amount ?? '—'}｜{row.error || row.duplicate}</label>{row.type && !row.error && account && <span>｜分類：<select aria-label={`第 ${row.rowNumber} 列分類`} value={row.categoryId} onChange={event => { const categoryId = event.currentTarget.value; setPreview(current => current.map(item => item.rowNumber === row.rowNumber ? updateImportPreviewRowCategory(item, categoryId, account, transactions) : item)); }}>{categoriesForTransactionType(row.type).map(category => <option key={category.id} value={category.id}>{category.name}</option>)}</select></span>}{suggestion && row.type && !row.error && account && <span>｜建議：{transactionCategoryLabel(suggestion.categoryId)}（{importCategorySuggestionReasonLabel(suggestion.reasonCodes[0])}）<button className="small" type="button" onClick={() => setPreview(current => current.map(item => item.rowNumber === row.rowNumber ? updateImportPreviewRowCategory(item, suggestion.categoryId, account, transactions) : item))}>套用建議</button></span>}</div>; })}</div><button className="small" type="button" disabled={selectedRowCount === 0} onClick={commit}>正式批次匯入已選列</button></>}
