...
import { reconcileMonthlyTransactions, type MonthlyTransactionReconciliation } from '../../lib/monthlyTransactionReconciliation';
import {
...
export default function ImportCenter({ accounts, transactions, sessions, presets, onCommit, onRollback, onPresets }: ImportCenterProps) {
  ...
  const makeReconciliation = () => {
    if (!account) return;
    try { setReconciliation(reconcileMonthlyTransactions(preview, transactions, account.id)); }
  ...
  return <div className="financial-account-list import-center">
    ...
    {preview.length > 0 && <button className="small" type="button" onClick={makeReconciliation}>產生對帳預覽</button>}
    {reconciliation && <section className="import-reconciliation-preview" aria-label="交易對帳預覽"><p className="note">對帳期間：{reconciliation.period.minDate} ～ {reconciliation.period.maxDate}（僅比較，不會修改帳本）</p><p className="note">已匹配 {reconciliation.summary.matched}｜可能相符 {reconciliation.summary.possible}｜Statement 未找到 {reconciliation.summary.statementOnly}｜App 未找到 {reconciliation.summary.appOnly}｜無效列 {reconciliation.summary.invalid}</p>{reconciliation.rows.filter(row => row.status !== 'matched').map(row => <p className="note" key={row.statementRow.rowNumber}>第 {row.statementRow.rowNumber} 列｜{row.status}｜{row.statementRow.description || '—'}</p>)}{reconciliation.appOnly.map(transaction => <p className="note" key={transaction.id}>App 未找到｜{transaction.occurredAt.slice(0, 10)}｜{transaction.description || transaction.merchant || '—'}</p>)}</section>}
