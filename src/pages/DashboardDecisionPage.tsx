...
export default function DashboardDecisionPage({ data }: { data: DashboardData }) {
  ...
  return <PageFrame page="home" title="投資決策首頁" description="30 秒掌握今日投資表現、配置與下一步。">
    ...
    <section className="dashboard-reminders-card" aria-labelledby="dashboard-reminders-title">
      ...
      {data.reminders.length === 0 ? <p className="dashboard-empty-state">目前沒有需要立即確認的報價、同步或配置提醒。</p> : <ul className="dashboard-reminder-list">{data.reminders.map(item => <li key={item.key} className={item.tone}><strong>{item.title}</strong><span>{item.detail}</span></li>)}</ul>}
    </section>
  </PageFrame>;
}
