...
export default function ClecStrategyCenterPage({ view, rule }: { view: ClecStrategyCenterResult; rule: ClecRuleOutput }) {
  ...
  return <PageFrame page="tools" title="CLEC 再平衡策略中心" description="查看目標配置來源、既有可計算方式與 CLEC 策略規格狀態；未核實公式不會試算或自動交易。">
    <section className="clec-center-grid">
      <article className="clec-card"><p className="eyebrow">目前配置來源</p><h2>{view.allocationSource.label}</h2><p className="note">正式目標配置目前只支援自訂比例；CLEC 433／442 已移至純模擬模板，不再是正式配置選項。類現金持股與銀行現金分開處理。</p><div className="clec-weights">{view.allocationSource.targetWeights.map(row => <p key={row.symbol}><span><b>{row.symbol}</b></span><strong>{pct(row.targetWeight)}</strong></p>)}</div><p><b>目標比例總和：</b>{pct(view.allocationSource.targetWeightTotal)}</p>{!view.allocationSource.rolesValid && <ul className="clec-list blocking">{view.allocationSource.blockingReasons.map(item => <li key={item}>{item}</li>)}</ul>}<Link className="clec-secondary-link" to="/assets">前往資產頁查看正式目標配置</Link><Link className="clec-secondary-link" to="/tools/allocation-simulator">如需依 CLEC 角色分類試算，請前往配置模擬器</Link></article>
...
const HAS_SIMULATION_TEMPLATE = new Set(['clec-smart-rebalance', 'annual-ratio-reset']);
