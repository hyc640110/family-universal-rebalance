...
const rawMoney = (value: number | null) => value === null || !Number.isFinite(value) ? '—' : `${(Math.abs(value) / 10000).toLocaleString('zh-TW', { maximumFractionDigits: 1 })} 萬元`;
...
export default function NetWorthHistoryPage({ history, snapshotView }: { history?: NetWorthSnapshot[]; snapshotView?: NetWorthSnapshotReadTimeView }) {
  ...
  return <PageFrame page="tools" title="淨資產歷史中心" description="每日快照自動統計資產、現金、負債與淨資產變化；不記錄交易流水。">
    ...
    <section className="history-stats">{cards.map(([label, value, format, isChange]) => <article key={label}><small>{label}</small><strong className={isChange ? changeTone(value) : 'hold'}>{format === 'percent' ? formatChangePercent(value) : isChange ? displayAmount(formatChangeMoney(value), hidden) : money(value)}</strong></article>)}</section>
