...
export default function NetWorthHistoryPage({ history }: { history?: NetWorthSnapshot[] }) {
  ...
  return <PageFrame page="tools" title="淨資產歷史中心" description="每日快照自動統計資產、現金、負債與淨資產變化；不記錄交易流水。">
    ...
    <section className="card net-worth-history-chart-card"><div className="history-range">{([['7d', '7 天'], ['30d', '30 天'], ['90d', '90 天'], ['1y', '1 年'], ['all', '全部']] as const).map(([value, label]) => <button type="button" key={value} className={range === value ? 'active' : ''} onClick={() => setRange(value)}>{label}</button>)}</div>{rows.length === 0 ? <div className="analytics-empty"><p>尚無淨資產歷史資料</p><span>開啟 App、更新報價或修改資產／借款資料時，系統會以當日資料覆蓋快照。</span></div> : <><TrendChart title="淨資產" unit="單位：萬元" data={rows.map(row=>({date:row.date,value:row.netWorth}))} formatValue={value=>money(value)} axisScale={10000}/><div className="history-legend"><span>淨資產趨勢</span><b>{money(rows.at(-1)?.netWorth ?? null)}</b></div></>}</section>
