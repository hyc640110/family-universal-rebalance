...
export default function AiDecisionCenterPage({ items, asOf, diagnostics }: { items: DecisionItem[]; asOf: string; diagnostics: HouseholdLiquidityDiagnosticPresentation }) {
  ...
  return <PageFrame page="tools" title="AI 決策中心" description="本地規則分析，不使用生成式 AI；每項結論均附判定依據。">
    <section className="ai-decision-intro"><div><p className="eyebrow">V5.6 Foundation</p><h2>資料摘要與規則判定</h2><p>資料日期：{asOf}。不預測股價、不提供買賣建議或報酬保證。</p></div></section>
    {today && <DecisionCard item={today} diagnostics={diagnostics} featured />}
    <section className="ai-decision-attention"><h2>需要留意</h2>{attention.length ? <div className="ai-decision-list">{attention.map(item => <DecisionCard item={item} diagnostics={diagnostics} key={item.id} />)}</div> : <p className="note">目前沒有 critical 或 warning 的規則判定。</p>}</section>
    <section className="ai-decision-grid">{items.filter(item => item.id !== 'today' && !attention.includes(item)).map(item => <DecisionCard item={item} diagnostics={diagnostics} key={item.id} />)}</section>
