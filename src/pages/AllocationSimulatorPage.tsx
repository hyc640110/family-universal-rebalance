...
export default function AllocationSimulatorPage({ rows, totalAssets, cash }: Props) {
  ...
  const result = useMemo(() => {
    const simulatedContribution = safeNumber(contribution);
  ...
  return <PageFrame page="tools" title="資產配置模擬器" description="調整目標配置與模擬投入金額，在不修改正式持股的情況下，預覽配置結果與再平衡方向。">
    ...
    {result.simulatedTotal <= 0 && <p className="warning-message">模擬後總資產必須大於 0，請調整抽回金額。</p>}
    <section className="card simulator-controls"><div><h2>模擬條件</h2><p className="note">正數代表額外投入，負數代表抽回；此欄位只影響目前頁面的模擬結果。</p></div><label>模擬投入金額（元）<input type="number" inputMode="decimal" value={contribution} onChange={event => setContribution(event.currentTarget.value)} /></label><div className="actions"><button type="button" className="small" onClick={() => setContribution('0')}>清除投入金額</button><button type="button" className="small" onClick={() => { setContribution('0'); resetTargets(); }}><RotateCcw size={16} /> 全部重設</button></div></section>
