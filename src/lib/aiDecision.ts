...
export function deriveAiDecisions(input: AiDecisionInput): DecisionItem[] {
  ...
  const common: DecisionItem[] = [
    ...
    { id: 'quote-freshness', category: 'quote-freshness' as const, severity: input.dashboard.holdingsCount === 0 ? 'unavailable' : input.quoteErrors || quoteUnknown || quoteUnavailable ? 'critical' : quoteStale || input.backupQuoteCount ? 'warning' : quoteRecent ? 'info' : 'normal', title: '股價更新狀態', conclusion: input.dashboard.holdingsCount === 0 ? '尚無持股，無需更新股價。' : input.quoteErrors ? '存在報價錯誤，建議更新股價資料。' : quoteUnknown ? '存在日期或時間不明報價，建議更新股價資料。' : quoteUnavailable ? '交易日資料未涵蓋，暫不將報價視為最新。' : quoteStale ? '存在過期報價，建議更新股價資料。' : quoteRecent ? '目前為最近有效交易日報價。' : '持股報價符合今日日期契約。', reason: input.dashboard.quoteStatus, evidence: [evidence('今日報價', String(quoteToday), 'quoteDateStatus'), evidence('最近有效', String(quoteRecent), 'quoteDateStatus'), evidence('過期／不明', `${quoteStale}／${quoteUnknown + quoteUnavailable}`, 'quoteDateStatus'), evidence('報價錯誤', String(input.quoteErrors), 'quoteDisplayStatus')], action: { label: '前往檢視', to: '/assets' } },
  ];
  const anomalies = [input.dashboard.holdingsCount === 0 ? '尚無持股' : '', input.quoteErrors ? '報價錯誤' : '', quoteUnknown ? '報價日期或時間不明' : '', quoteUnavailable ? '交易日資料未涵蓋' : '', input.targetOverLimit ? '目標比例合計超過 100%' : '', investment === null ? '投資資產數值無效' : '', totalAssets !== null && totalAssets === 0 && input.holdingMarketValue > 0 ? '總資產與持股市值互相矛盾' : '', marketFreshness === 'unavailable' ? '市場資料 unavailable' : '', input.performance.snapshotCount < 2 ? '投資績效快照不足' : '', input.dividend.summary.yearCount === 0 ? '股息有效資料不足' : ''].filter(Boolean);
