...
export function parseMarketSnapshot(raw: unknown, cacheControl: string | null = null): MarketSnapshot {
...
export const marketRefreshRequestInit = (manual = false): RequestInit => ({ cache: manual ? 'no-store' : 'default', headers: { accept: 'application/json' } });
export async function fetchMarketSnapshot(endpoint: string, options: { signal?: AbortSignal; manual?: boolean; requestId?: number } = {}): Promise<MarketSnapshot> { if (!endpoint) return buildUnavailableMarketSnapshot(); try { const url = new URL(`${endpoint.replace(/\/$/, '')}/market-summary`); if (options.manual) { url.searchParams.set('refresh', '1'); url.searchParams.set('request', String(options.requestId ?? Date.now())); } const response = await fetch(url.toString(), { signal: options.signal, ...marketRefreshRequestInit(options.manual) }); if (!response.ok) return buildUnavailableMarketSnapshot(`市場資料服務暫時無法取得（HTTP ${response.status}）。`); return parseMarketSnapshot(await response.json(), response.headers.get('cache-control')); } catch { return buildUnavailableMarketSnapshot('市場資料服務暫時無法連線。'); } }
