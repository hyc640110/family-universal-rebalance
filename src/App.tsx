...
import { createRebalanceDecisionSnapshot, normalizeRebalanceDecisionJournal, type RebalanceDecision, type RebalanceDecisionSnapshot } from './lib/rebalanceDecisionJournal';
...
import { holdingHistorySnapshotFromRows, holdingHistorySnapshotsEqual, normalizeHoldingHistory, upsertHoldingHistorySnapshot, type HoldingHistorySnapshot } from './lib/holdingHistory';
...
// UR-TODO-054-B: FX Confirmation UI. deriveFxConversionPresentations() is the read-only selector
// (mirrors deriveLoanRepaymentGroupPresentations() below); confirmFxConversionAndAppend() is the
// existing, unmodified F2D contract entry point — this file only resolves the envelope and wires
// the result into state, never re-implements any part of the confirmation contract itself.
import { deriveFxConversionPresentations } from './lib/fxConversionPresentation';
...
const defaultState: AppState = {
  ...
  // UR-TODO-070: display-only ordering for active holding cards; brand-new installs have no
  // preference yet, so this is simply DEFAULT_HOLDINGS' own order (normalizeHoldingDisplayOrder
  // produces the identical result on first normalizeState() pass — this literal only matters for
  // readStateWithSnapshotView()'s "no localStorage at all" early-return path, same as focusedSymbols above).
  holdingDisplayOrder: DEFAULT_HOLDINGS.map(h => h.symbol),
...
function DraftInput({ value, type = 'text', min, step, inputMode, onCommit }: { value: string | number; type?: string; min?: string; step?: string; inputMode?: React.HTMLAttributes<HTMLInputElement>['inputMode']; onCommit: (value: string) => void }) {
  ...
  const onFocus = () => { editingRef.current = true; setEditing(true); if (draftRef.current === '0') updateDraft(''); };
  return <input type={type} min={min} step={step} inputMode={inputMode} value={draft} onFocus={onFocus} onInput={e => updateDraft(e.currentTarget.value)} onChange={e => updateDraft(e.currentTarget.value)} onBlur={e => commit(e.currentTarget.value)} onKeyDown={e => { if (e.key === 'Enter') commit(e.currentTarget.value); }} />;
}
...
function App() {
  ...
  const isRetirementPlanner = routeLocation.pathname === '/tools/retirement-planner';
  ...
  // UR-TODO-078 Phase A: App-observed per-holding snapshot, built from the same calculateMetrics()
  // rows as currentNetWorthSnapshot above — no second quote fetch, no polling, no Worker change.
  const currentHoldingHistorySnapshot = useMemo(() => holdingHistorySnapshotFromRows(m.rows.map(row => ({
    symbol: row.symbol,
    shares: row.shares,
  ...
  // UR-TODO-078 Phase A Foundation only — no Consumer UI reads state.holdingHistory yet.
  // The equality bail-out must happen BEFORE calling setState (see the dipAlerts effect's comment
  // above): setState() unconditionally re-normalizes and commits a new object on every call, so an
  // equality check placed inside its updater never actually prevents a re-render.
  useEffect(() => {
    if (!hasUpdatedQuotes) return;
    const current = stateRef.current;
    const latest = current.holdingHistory?.find(snapshot => snapshot.date === currentHoldingHistorySnapshot.date);
    if (holdingHistorySnapshotsEqual(latest, currentHoldingHistorySnapshot)) return;
    setState({ ...current, holdingHistory: upsertHoldingHistorySnapshot(current.holdingHistory, currentHoldingHistorySnapshot) });
  }, [currentHoldingHistorySnapshot, hasUpdatedQuotes]);
  ...
  // UR-TODO-054-B: read-only presentation only; collapses the two independently-reconciled FX
  // conversion legs into one row per conversionId for FxConfirmationCard. Reuses the same
  // state.opaqueTransactions/state.transactions/state.financialEvents already computed above.
  const fxConversionPresentations = useMemo(() => deriveFxConversionPresentations({
    opaqueTransactions: state.opaqueTransactions,
    transactions: state.transactions,
    ledgerEvents: state.financialEvents
  }), [state.opaqueTransactions, state.transactions, state.financialEvents]);
  ...
  const recordRebalanceDecision = (decision: RebalanceDecision, note: string) => {
    if (!rebalanceRecommendationView.canRecommend) return;
    const timestamp = now();
    const snapshot = createRebalanceDecisionSnapshot({
      id: uid(), createdAt: timestamp, asOfDate: localSnapshotDate(), decidedAt: timestamp, decision, note: note.trim() || null,
      recommendation: rebalanceRecommendationView,
      quoteEvidence: m.rows.map(row => ({
        symbol: row.symbol,
        price: Number.isFinite(row.quote.price) ? row.quote.price : null,
        quoteDate: row.quote.quoteDate ?? null,
        quoteTime: row.quote.quoteTime ?? null,
        quoteStatus: quoteDateStatus(row.quote.quoteDate, row.quote.quoteTime),
        quoteSource: row.quote.source || null,
        quoteError: row.quote.error ?? null,
      })),
    });
    setState(current => ({ ...current, rebalanceDecisionJournal: [...(current.rebalanceDecisionJournal ?? []), snapshot] }));
  };
  ...
  // UR-TODO-054-B: the sole write path from the FX Conversion Confirmation UI to
  // state.financialEvents. Reuses the existing, unmodified confirmFxConversionAndAppend()
  // contract wholesale — this handler only resolves the envelope for the given conversionId; it
  // never re-implements any part of the confirmation contract itself.
  const confirmFxConversion = (conversionId: string): FxConfirmOutcome => {
    const current = stateRef.current;
    const envelope = current.opaqueTransactions.find(candidate => candidate.id === conversionId);
    if (!envelope) return { rejected: true, reason: '找不到對應的換匯候選資料，可能已被刪除或修改，請重新整理後再試一次。' };
    const opaqueTransactionsById = new Map(current.opaqueTransactions.map(candidate => [candidate.id, candidate]));
    ...
    try {
      ...
      // Defense-in-depth: mirrors confirmLoanRepaymentGroup above — any unforeseen throw anywhere
      // in the confirmation chain must still surface as a visible rejection, never a silent no-op.
      return { rejected: true, reason: `確認時發生未預期的錯誤，未寫入任何記帳資料，請重新整理頁面後再試一次。${error instanceof Error ? `（${error.message}）` : ''}` };
    }
    if (result.rejected) return result;
    // CRITICAL, and the opposite of confirmLoanRepaymentGroup above: unlike Loan's
    // LoanPaymentConfirmationResult (which returns only the newly-built component group),
    // ConfirmFxConversionResult's `events` on success is the FULL already-appended Ledger (see
    // confirmFxConversionAndAppend's own doc comment — it calls appendFinancialEvent() internally
    // and returns `appended.events`, not `built`). This must REPLACE state.financialEvents
    // wholesale here, never be spread onto the existing array — doing so would duplicate the
    // entire Ledger.
    setState(current => ({ ...current, financialEvents: result.events }));
    return { rejected: false };
  };
  // UR-TODO-054-B: void for an FX conversion reuses the existing, unmodified
  // voidFinancialEventAndAppend() primitive verbatim, exactly like Loan's voidLoanRepaymentGroup
  // above — but simpler, since resolveActiveFxConversionGroups() guarantees at most one active
  // event per conversionId, so voiding "the" event is unambiguous (no deterministic-target
  // selection like Loan's multi-component group needs).
  const voidFxConversion = (eventId: string): FxVoidOutcome => {
    const result = voidFinancialEventAndAppend(stateRef.current.financialEvents, { eventId, now: now() });
    if (result.rejected) return result;
    setState(current => ({ ...current, financialEvents: result.events }));
    return { rejected: false };
  ...
  if (routeLocation.pathname === '/') return <Navigate to="/home" replace />;
    if (!validPages.includes(currentPage) && !isAllocationSimulator && !isRebalanceStrategyComparison && !isRiskCenter && !isWealthGoal && !isRetirementPlanner && !isCashFlowCenter && !isNetWorthHistory && !isDividendCenter && !isAiDecisionCenter && !isPortfolioRiskCenter && !isRebalanceRecommendationCenter && !isClecStrategyCenter && !isInvestmentActionCenter) return <Navigate to="/home" replace />;
  ...
  return (
    <AppLayout>
      ...
      {isRiskCenter && <RiskCenterPage input={riskInput} diagnostics={householdLiquidityDiagnosticPresentation} today={localSnapshotDate()} onConfirmLoanFresh={confirmLoanDataFresh} />}
       ...
       {isRetirementPlanner && <RetirementPlannerPage plan={state.retirementPlan} cashFlowProfile={state.cashFlowProfile} currentNetWorth={m.totalAssets - m.debt} onSave={retirementPlan => setState(s => ({ ...s, retirementPlan }))} />}
      ...
      {isNetWorthHistory && <NetWorthHistoryPage history={netWorthHistory} snapshotView={netWorthSnapshotReadTimeViewRef.current} />}
        ...
        {isRebalanceRecommendationCenter && <RebalanceRecommendationPage view={rebalanceRecommendationView} recommendations={recommendationModels} rule={clecStrategyRuleView} eligibility={rebalanceExecutionEligibility} journal={state.rebalanceDecisionJournal ?? []} onRecordDecision={recordRebalanceDecision} />}
