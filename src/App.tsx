...
function CashList({ items, setItems }: { items: CashItem[]; setItems: (items: CashItem[]) => void }) {
  return <div className="list cash-list"><div className="list-row list-head"><span>名稱</span><span>金額（萬元）</span><span>備註</span><span>操作</span></div>{items.map(item => <div className="list-row" key={item.id}><label><span>名稱</span><input value={item.name} onChange={e => setItems(items.map(x => x.id === item.id ? { ...x, name: e.target.value } : x))} /></label><label><span>金額（萬元）</span><input type="number" value={item.amount / 10000} onChange={e => setItems(items.map(x => x.id === item.id ? { ...x, amount: Math.max(0, num(e.target.valueAsNumber) * 10000) } : x))} /></label><label><span>備註</span><input value={item.note} onChange={e => setItems(items.map(x => x.id === item.id ? { ...x, note: e.target.value } : x))} /></label><button className="danger small" onClick={() => setItems(items.filter(x => x.id !== item.id))}>刪除</button></div>)}<button className="small" onClick={() => setItems([...items, { id: uid(), name: '現金', amount: 0, note: '' }])}>新增</button></div>;
}
...
function App() {
  ...
  const uploadCloud = async (label = 'Firebase 同步完成') => { setSync('上傳中…'); const normalized = normalizeState(state); setState(normalized); await uploadFirebase(normalized.firebase, normalized); setSync(`${label} ${tw(now())}｜${syncPath(normalized.firebase)}｜持股 ${normalized.holdings.length} 筆`); };
  const downloadCloud = async () => { setSync('下載中…'); const remote = await downloadFirebase(state.firebase); setState(remote); setSync(`Firebase 同步完成 ${tw(now())}｜${syncPath(remote.firebase)}｜持股 ${remote.holdings.length} 筆`); };
        ...
        <Card title="持股配置">
          <div className="holdings">
            {m.rows.map(r => { const pnlPct = r.cost ? r.pnl / r.cost * 100 : 0; return <article className="holding" key={r.symbol}>
              ...
              {r.symbol === '00631L' ? <label>目標比例 %<input type="text" inputMode="decimal" value={targetDraft} onChange={e => setTargetDraft(e.target.value)} onBlur={commitTarget} onKeyDown={e => { if (e.key === 'Enter') e.currentTarget.blur(); }} /><small>限制 {MIN_GROWTH_TARGET}%～{MAX_GROWTH_TARGET}%，防守資產自動為 {pct(m.defensiveTargetPct)}。</small></label> : <p className="note">列入防守資產，不參與主動再平衡買賣。</p>}
