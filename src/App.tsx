...
const defaultState: AppState = {
  ...
  firebase: { databaseURL:'', secretPath:'00631l-pro-v61' }, workerUrl:'', autoSync:false, autoSyncSec:60
};
...
function App(){
  ...
  const uploadCloud=async(label='已上傳')=>{ await syncFirebase(state.firebase,state); setSync(`${label} ${tw(now())}`); };
  const downloadCloud=async()=>{ const remote=await syncFirebase(state.firebase); if(remote)setState({...defaultState,...remote}); setSync('已下載 '+tw(now())); };
  ...
  useEffect(()=>{ if(!state.autoSync || !state.firebase.databaseURL) return; const id=setTimeout(()=>uploadCloud('自動同步完成').catch(e=>setSync('自動同步失敗 '+e.message)), Math.max(10,state.autoSyncSec)*1000); return()=>clearTimeout(id); },[state]);
