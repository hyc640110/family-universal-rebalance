...
import { IMPORT_SCHEMA_VERSION, MAX_IMPORT_FILE_BYTES, MAX_IMPORT_ROWS, buildImportPreview, createImportSessionId, createImportTransactions, csvParse, importedBySession, rowsToRecords, type ImportMapping, type ImportPreviewRow, type ImportPreset, type ImportSession } from './lib/importCenter';
...
function normalizeState(raw: unknown): AppState {
  ...
  const importPresets = Array.isArray(r.importPresets) ? r.importPresets.filter(value => value && typeof value === 'object').slice(-20) as ImportPreset[] : [];
  ...
  return (
    <AppLayout>
      ...
      {showOn('assets', 'analytics') && <DashboardPage>
        ...
        <SectionCard className="page-card for-assets" id="transactions-section" title="交易基礎" isMobile={isMobile} collapsible open={sectionOpen('transactions')} onToggle={() => toggleSection('transactions')} summary={`${state.transactions.length} 筆交易`}><TransactionList accounts={state.accounts} transactions={state.transactions} onCreate={createTransaction} onDelete={deleteTransaction} onUpdate={updateTransaction} /><ImportCenter accounts={state.accounts} transactions={state.transactions} sessions={state.importSessions} onCommit={commitImport} onRollback={rollbackImport} /></SectionCard>
