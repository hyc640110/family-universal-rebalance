...
import { TRANSACTION_SCHEMA_VERSION, accountHasTransactions, createTransactionId, createTransferTransaction, deriveTransactionAccountBalances, normalizeTransactions, transactionCashFlowSummary, validateTransferAccounts, type FinancialTransaction, type TransactionStatus, type TransactionType } from './lib/transactions';
...
function TransactionList({ accounts, transactions, onCreate, onDelete, onUpdate }: { accounts: FinancialAccount[]; transactions: FinancialTransaction[]; onCreate: (input: Pick<FinancialTransaction, 'accountId' | 'transferAccountId' | 'type' | 'amount' | 'occurredAt' | 'note' | 'status' | 'excluded'>) => void; onDelete: (id: string) => void; onUpdate: (id: string, patch: Partial<FinancialTransaction>) => void }) {
  const [accountId, setAccountId] = useState(''); const [transferAccountId, setTransferAccountId] = useState(''); const [type, setType] = useState<TransactionType>('expense'); const [amount, setAmount] = useState(''); const [occurredAt, setOccurredAt] = useState(new Date().toISOString().slice(0, 10)); const [note, setNote] = useState(''); const [status, setStatus] = useState<TransactionStatus>('posted'); const [message, setMessage] = useState(''); const [accountFilter, setAccountFilter] = useState(''); const [typeFilter, setTypeFilter] = useState<'all' | TransactionType>('all');
  const active = accounts.filter(account => account.isActive); const summary = transactionCashFlowSummary(transactions);
  const add = () => { const numeric = Number(amount); if (type === 'transfer') { const error = validateTransferAccounts(accountId, transferAccountId, numeric, active); if (error) { setMessage(error); return; } } else if (!accountId || !(numeric > 0)) { setMessage('請選擇帳戶並輸入大於 0 的金額'); return; } onCreate({ accountId, transferAccountId: type === 'transfer' ? transferAccountId : undefined, type, amount: numeric, occurredAt: `${occurredAt}T00:00:00.000Z`, note, status, excluded: false }); setAmount(''); setNote(''); setMessage(''); };
...
function App() {
  ...
  const createTransaction = (input: Pick<FinancialTransaction, 'accountId' | 'transferAccountId' | 'type' | 'amount' | 'occurredAt' | 'note' | 'status' | 'excluded'>) => setState(current => { const timestamp = now(); const account = current.accounts.find(item => item.id === input.accountId); if (!account) return current; if (input.type === 'transfer') { try { return { ...current, transactions: [...current.transactions, createTransferTransaction({ accountId: input.accountId, transferAccountId: input.transferAccountId, status: input.status, source: 'manual', amount: input.amount, currency: account.currency, note: input.note, occurredAt: input.occurredAt, excluded: input.excluded }, current.accounts, timestamp)] }; } catch { return current; } } return { ...current, transactions: [...current.transactions, { id: createTransactionId(), accountId: input.accountId, type: input.type, status: input.status, source: 'manual', amount: Math.max(0, input.amount), currency: account.currency, categoryId: input.type === 'income' ? 'income-other' : 'expense-other', note: input.note, occurredAt: input.occurredAt, fingerprint: '', excluded: input.excluded, createdAt: timestamp, updatedAt: timestamp }] }; });
  ...
  const updateTransaction = (id: string, patch: Partial<FinancialTransaction>) => setState(current => ({ ...current, transactions: current.transactions.map(transaction => transaction.id === id ? { ...transaction, ...patch, id: transaction.id, updatedAt: now() } : transaction) }));
  ...
  return (
    <AppLayout>
      ...
      {showOn('assets', 'analytics') && <DashboardPage>
        ...
        <SectionCard className="page-card for-assets" title="帳戶管理" isMobile={isMobile} collapsible open={sectionOpen('cash')} onToggle={() => toggleSection('cash')} summary={`可用現金 ${money(m.cash)}｜${state.accounts.filter(account => account.isActive).length} 個啟用帳戶`}>
        ...
        <SectionCard className="page-card for-assets" title="交易基礎" isMobile={isMobile} collapsible open={sectionOpen('cash')} summary={`${state.transactions.length} 筆交易`}><TransactionList accounts={state.accounts} transactions={state.transactions} onCreate={createTransaction} onDelete={deleteTransaction} onUpdate={updateTransaction} /></SectionCard>
