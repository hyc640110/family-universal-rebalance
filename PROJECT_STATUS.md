...
- V4.1 淨資產歷史中心（每日淨資產快照與趨勢）
- V4.1 FinancialAccount Foundation：型別安全帳戶模型、穩定 ID、CashItem 單次遷移、手動／衍生餘額模式、帳戶 CRUD、localStorage／Firebase／JSON Backup 相容，以及防止舊現金資料重複計入淨資產。
- V4.2 Transaction Foundation：交易模型、帳戶關聯、手動交易 CRUD、轉帳資料模型、交易衍生餘額、交易 fingerprint、類別基礎與持久化相容。
...
1. V4.2 Draft PR 驗證完成後由使用者手動 Merge；不得開始 V4.3 Import Center 直到 V4.2 合併。
2. V4.3 Import Center（待 V4.2 合併後另開 Sprint）。
