...
const checks = [
  ...
  ,['Dashboard uses existing quote, risk and history sources without inventing finance data', /deriveHistoryStats/.test(app) && /deriveInvestmentDashboard/.test(app) && /資產快照/.test(dashboard) && /狀態確認/.test(dashboard) && /net-worth-history/.test(dashboard)]
