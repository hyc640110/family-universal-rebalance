...
const checks = [
  ...
  ,['Dashboard uses existing quote, risk and history sources without inventing finance data', /deriveHistoryStats/.test(app) && /deriveInvestmentDashboard/.test(app) && /今日投資摘要/.test(dashboard) && /投資健康度/.test(dashboard) && /狀態確認/.test(dashboard) && /net-worth-history/.test(dashboard) && /tools\/risk-center/.test(dashboard)]
