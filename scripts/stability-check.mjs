...
const checks = [
  ...
  ,['Tool quick navigation follows the shared Tool Center order', /TOOL_DEFINITIONS[\s\S]*dividend-center[\s\S]*ai-decision[\s\S]*portfolio-risk[\s\S]*rebalance-recommendation[\s\S]*clec-strategy[\s\S]*wealth-goal[\s\S]*cash-flow[\s\S]*net-worth-history[\s\S]*allocation-simulator[\s\S]*risk-center/.test(readFileSync('src/lib/toolNavigation.ts','utf8'))]
