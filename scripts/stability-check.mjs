...
const checks = [
  ...
  ['Full mode sections remain manually collapsible on desktop while mobile derives compact defaults', /const effectiveDisplayMode = isMobile \? 'compact' : uiState\.displayMode/.test(app) && /const defaultSectionsForMode = effectiveDisplayMode === 'full' \? FULL_UI_SECTIONS : DEFAULT_UI_STATE\.sections/.test(app)],
  ...
  ['Allocation simulator is read-only session state', /useState<Record<string, string>>/.test(simulator) && !/localStorage\.setItem|writeState\(|uploadFirebase\(|downloadFirebase\(/.test(simulator)],
  ...
  ,['Net worth history page includes ranges, chart and statistics', /7 天/.test(netWorthHistoryPage) && /最大回撤/.test(netWorthHistoryPage) && /TrendChart/.test(netWorthHistoryPage)]
  ...
  ,['Allocation simulator keeps safety-cash use session-only and explicitly warned', /deriveAllocationSimulatorFunding/.test(simulator) && /const \[allowSafetyCashUsage, setAllowSafetyCashUsage\] = useState\(false\)/.test(simulator) && /假設動用安全現金/.test(simulator) && /role="alert" aria-atomic="true"/.test(simulator) && !/localStorage\.setItem|writeState\(|uploadFirebase\(|downloadFirebase\(/.test(simulator) && !/setContribution|simulatedContribution|模擬投入金額/.test(simulator)]
  ,['Display mode uses one persisted desktop UI state while mobile derives compact sections', /writeUiState\(uiState\)/.test(app) && /effectiveDisplayMode === 'full' \? FULL_UI_SECTIONS : DEFAULT_UI_STATE\.sections/.test(app) && /document\.documentElement\.dataset\.displayMode = effectiveDisplayMode/.test(app)]
  ,['Compact and full modes have distinct user-facing descriptions', /只顯示核心資訊，適合日常快速查看。/.test(app) && /顯示完整分析、進階欄位與說明。/.test(app)]
  ,['Display mode only controls remaining collapsible defaults without hiding functionality', /JSON\.stringify\(\{ displayMode: state\.displayMode \}\)/.test(app) && !/data-display-mode="compact"/.test(styles) && /quoteSources: false[\s\S]*targetCheck: false/.test(app) && /quoteSources: true[\s\S]*targetCheck: true/.test(app) && !/syncStatus|syncDiagnostics/.test(app)]
  ,['Settings details reuse SectionCard, surface target errors, and omit retired sync details', /id="quote-sources-section"[\s\S]*SectionCard/.test(app) && /id="target-check-section"[\s\S]*targetCheckHasError/.test(app) && /setUiState\(current => current\.sections\.targetCheck/.test(app) && !/sync-status-section|sync-diagnostics-section/.test(app)]
  ,['Dashboard uses existing quote, risk and history sources without inventing finance data', /deriveHistoryStats/.test(app) && /deriveInvestmentDashboard/.test(app) && /今日投資摘要/.test(dashboard) && /狀態確認/.test(dashboard) && /net-worth-history/.test(dashboard)]
  ,['Dashboard health card is fully removed and reminders card only renders when there is something to confirm', !/投資健康度/.test(dashboard) && !/dashboard-health-card/.test(dashboard) && /data\.reminders\.length > 0 && <section className="dashboard-reminders-card"/.test(dashboard)]
  ...
  ,['Account section uses its persisted toggle while transaction tools remain anchor-accessible', /id="accounts-section"[\s\S]*sectionOpen\('cash'\)[\s\S]*toggleSection\('cash'\)/.test(app) && /showTransactionTools = isTransactionToolsTarget\(routeLocation\.pathname, routeLocation\.hash\)/.test(app) && /\{showTransactionTools && <SectionCard[\s\S]*id="transactions-section"/.test(app) && /transactions: false/.test(app) && /transactions: true/.test(app)]
