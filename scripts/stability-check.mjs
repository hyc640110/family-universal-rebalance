...
const checks = [
  ...
  ,['Allocation simulator uses formal funding without a local contribution input', /deriveAllocationSimulatorFunding/.test(simulator) && /allowSafetyCashUsage: false/.test(simulator) && /模擬資金來源/.test(simulator) && !/setContribution|simulatedContribution|模擬投入金額/.test(simulator)]
