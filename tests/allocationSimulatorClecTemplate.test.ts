...
import { deriveAllocationPresetPreview } from '../src/lib/allocationPresets';
...
test('AllocationSimulatorPage 的樣板套用只呼叫既有 deriveAllocationPresetPreview 並覆寫 local targets，不寫入正式配置', () => {
  ...
  assert.match(page, /const applyTemplatePreview = \(\) => \{\s*if \(!templatePreview\.canApply\) return;\s*setTargets\(current => \(\{ \.\.\.current, \.\.\.Object\.fromEntries\(templatePreview\.rows\.map\(row => \[row\.symbol, String\(row\.nextWeight \?\? 0\)\]\)\) \}\)\);\s*\};/);
...
test('套用樣板不影響既有「恢復正式目標比例」與資金基數計算邏輯', () => {
  ...
  assert.match(page, /const resetTargets = \(\) => setTargets\(Object\.fromEntries\(rows\.map\(row => \[row\.symbol, String\(officialTarget\(row\)\)\]\)\)\);/);
