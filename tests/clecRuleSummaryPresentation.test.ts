...
test('uses the existing CLEC route and the rebalance page only receives the app-derived output', () => {
  ...
  assert.match(app, /RebalanceRecommendationPage view=\{rebalanceRecommendationView\} recommendations=\{recommendationModels\} rule=\{clecStrategyRuleView\} eligibility=\{rebalanceExecutionEligibility\}/); assert.doesNotMatch(page, /deriveClecStrategyRule|buildClecStrategyRuleInput/); assert.match(page, /<ClecRuleSummaryCard rule=\{rule\} \/>/); assert.match(css, /\.clec-rule-summary-bridge/); assert.match(css, /@media \(max-width:700px\)\{\.clec-rule-summary-bridge/);
});
