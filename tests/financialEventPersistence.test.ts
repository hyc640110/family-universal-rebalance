...
test('localStorage 正規化與 JSON Backup 匯出匯入保留有效 Ledger', async () => {
...
test('v2 Loan component group 在 localStorage 與 JSON Backup 中加法式保存；舊 Ledger 不需要 migration', async () => {
  const { backupPayload, normalizeState, stateFromBackup } = await loadAppPersistence();
...
test('v3 generic split group 在 localStorage 與 JSON Backup 中完整 round-trip', async () => {
  const { backupPayload, normalizeState, stateFromBackup } = await loadAppPersistence();
...
test('P2-A App 不再持有 Firebase download、upload 或 remote Ledger merge runtime caller', async () => {
  const source = await (await import('node:fs/promises')).readFile(new URL('../src/App.tsx', import.meta.url), 'utf8');
  for (const name of ['stateFromFirebasePayload', 'uploadFirebaseStateWithLedgerMerge', 'fetchRemoteFinancialEventLedger', 'downloadFirebase', 'uploadCloud', 'downloadCloud']) {
    assert.equal(source.includes(name), false, `${name} must not remain an active App runtime caller`);
  }
});
...
test('舊資料在 localStorage 與 JSON Backup 讀取路徑維持空 Ledger，不自動將歷史交易變成事件', async () => {
  const { backupPayload, normalizeState, stateFromBackup } = await loadAppPersistence();
  ...
  for (const state of [legacy, restoredLegacy]) {
