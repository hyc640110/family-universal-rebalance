...
test('Dashboard integration keeps a single status card, existing routes, and safe wording', () => {
  const page = readFileSync(new URL('../src/pages/DashboardDecisionPage.tsx', import.meta.url), 'utf8');
  const app = readFileSync(new URL('../src/App.tsx', import.meta.url), 'utf8');
  const css = readFileSync(new URL('../src/styles.css', import.meta.url), 'utf8');
  assert.match(app, /adaptInvestmentIntelligenceInput/); assert.match(page, /investment-intelligence-card/);
  assert.match(page, /intelligence\.overallStatus/); assert.match(page, /intelligence\.summary/);
  assert.match(css, /@media \(max-width:700px\)\{\.investment-intelligence-card/);
