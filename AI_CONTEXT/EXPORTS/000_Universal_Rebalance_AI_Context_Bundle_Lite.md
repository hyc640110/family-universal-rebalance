...
Generated UTC: 2026-08-05T08:28:39.587734+00:00
...
- `003_CURRENT_STATUS.md` — SHA-256 `32068c1c1fa2f3d408bef9849fc27039adce29e8b9a995305c8229903830cb37`
- `008_TODO_BACKLOG.md` — SHA-256 `d581336019b4dfaa5a80a2ba1ee787e42fed5a9cb7a95675a55a69df4928885f`
...
2026-08-05 更新：**UR-TODO-046 C1、046-B、046-C1／C2、046-C3A、046-C3B、046-C3C-A 與 046-C3C-B 已完成，但 UR-TODO-046 整體仍未完成**。後續 Ledger 寫入／持久化（C3C-C）、Firebase Ledger sync 等仍需獨立排程與產品決策（皆屬重大事件）。

- 優先級：待評估
- 狀態：**部分完成／後續待評估**（Phase 1 唯讀盤點、C1 Financial Event Ledger contract／persistence foundation、046-B pure attribution calculator／quality model、046-C1／C2 pure transaction reconciliation、046-C3A pure runtime derived-evidence adapter、046-C3B runtime attribution composition layer、046-C3C-A runtime attribution provenance card 與 046-C3C-B session-only mark-as-reasonable toggle 已完成；UR-TODO-043-B 依賴已解除；其餘子階段（C3C-C Ledger 寫入／持久化、Firebase Ledger sync 等重大事件）尚未排程或開發）
- 提出日期：2026-07-30
...
- **Remaining Boundaries**：**046-C3C-C（Ledger 寫入／持久化，將使用者確認正式落地為 `FinancialEvent`）是下一潛在候選，但未經授權不得開始**——需要使用者對「何時、如何把 session-only 標示轉換為正式記帳」另行拍板，屬於 schema／persistence／Ledger 寫入層級的重大事件。「本次已標示 N 筆」彙總計數本次刻意未做，維持延後決議。`state.financialEvents` 目前沒有任何 CRUD UI，Ledger evidence 在實務上通常為空，卡片目前主要呈現「衍生證據」與「未解釋殘差」；這不是 C3C-A／C3C-B 範圍要處理的缺口，但值得記錄供未來排程參考。
- **下一正式候選（待盤點，未開始）**：046-C3C-C Ledger 寫入／持久化、Firebase Financial Event Ledger sync、split allocation、investment buy／sell attribution、loan principal／interest attribution、FX attribution，或其他最新治理文件已定義項目，需依最新 Repository／AI_CONTEXT 另行唯讀判斷。**若下一候選涉及 Ledger 寫入、schema／persistence 變更、核心 attribution 結果改變、AI Decision／Rebalance 接線或啟用 Firebase Ledger sync，屬重大產品／核心財務語意事件，須另行拍板，不得自動開始。** UR-TODO-046 整體維持「部分完成／後續待評估」，不得因 C3C-A／C3C-B 完成而自行標記整體已完成。
