...
Generated UTC: 2026-08-07T16:19:15.734585+00:00
...
- `003_CURRENT_STATUS.md` — SHA-256 `13a338115a1f177d2255945cc0ac972a81d2ed90fe95fbe27d11563b2889538c`
- `008_TODO_BACKLOG.md` — SHA-256 `d042cf581b8178c89c5c6dc8232a9d2d5e2d1c162635c835c2953f2343e1f82b`
...
# Universal Rebalance Todo Backlog v1.72
...
2026-08-05 更新：**UR-TODO-046 C1、046-B、046-C1／C2、046-C3A、046-C3B、046-C3C-A、046-C3C-B 與 046-C3C-C 已完成，但 UR-TODO-046 整體仍未完成**。後續撤銷／void、Firebase Ledger sync、split allocation、investment buy／sell attribution、loan principal／interest attribution、FX attribution 等仍需獨立排程與產品決策（皆屬重大事件），詳見下方「Remaining Boundaries」。

- 優先級：待評估
- 狀態：**部分完成／後續待評估**（Phase 1 唯讀盤點、C1 Financial Event Ledger contract／persistence foundation、046-B pure attribution calculator／quality model、046-C1／C2 pure transaction reconciliation、046-C3A pure runtime derived-evidence adapter、046-C3B runtime attribution composition layer、046-C3C-A runtime attribution provenance card、046-C3C-B session-only mark-as-reasonable toggle 與 046-C3C-C Financial Event Ledger 寫入／持久化已完成；UR-TODO-043-B 依賴已解除；其餘子階段（撤銷／void、Firebase Ledger sync、split allocation、investment buy／sell attribution、loan principal／interest attribution、FX attribution 等重大事件）尚未排程或開發）
- 提出日期：2026-07-30
...
- **Remaining Boundaries（C3C-C 完成後更新）**：
  - **撤銷／void／取消確認功能**：本次明確排除，目前只能新增、不能撤銷已確認的記帳；為下一潛在候選，須另行拍板。
  - **跨裝置衝突處理／Firebase Ledger sync**：`financialEvents` 目前完全不經 Firebase 同步（延續 C1 既有決策），本機與其他裝置之間沒有合併機制；待未來 Google 登入／帳號升級（UR-TODO-001 的下一候選）評估時一併考慮，屬重大事件。
  - **批次確認**：本次維持逐筆，未實作批次。
