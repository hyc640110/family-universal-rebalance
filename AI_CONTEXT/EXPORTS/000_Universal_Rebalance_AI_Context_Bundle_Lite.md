...
**UR-TODO-078 Per-Holding Historical Snapshot Foundation Phase A 已正式 CLOSED／Production Verified（2026-08-23）。** PR [#425](https://github.com/hyc640110/family-universal-rebalance/pull/425) final head `9264fb6a46755cc70e8471541c894e8aeb5178c6` 已由 `hyc640110` 於 `2026-08-23T16:49:39Z` 以一般 2-parent merge commit `350da693c1faa5fa12813e969f146e3d273b2038`（parents `adc69021dd35c68a3916c4e746be3379a18fcdcf`／`9264fb6a46755cc70e8471541c894e8aeb5178c6`；未使用 admin override）合併；`origin/main` 正式基線更新為 `350da693c1faa5fa12813e969f146e3d273b2038`。merge 後 main push 觸發之 Deploy GitHub Pages run [32652884649](https://github.com/hyc640110/family-universal-rebalance/actions/runs/32652884649) success（`build`／`deploy` 皆 success，`build` 內 regression gate 通過，headSha 與 merge commit 一致）。Production 已唯讀確認：HTTP 200、deployment sha 一致、JS `index-ZiKPt2JD.js`／CSS `index-hMnE973C.css` 與本機最終 build 產物一致；以全新瀏覽器分頁驗證首頁與 `#/assets` 頁完整渲染，資產配置四張卡片維持 UR-TODO-076 既有 fail-closed「近1個月趨勢資料不足」、**Consumer sparkline 未出現**、持股報價正常、375px 無 horizontal overflow、console 0 error；另唯讀下載 Production bundle 確認 `holdingHistory`／`quoteAvailable` 欄位已隨部署生效，驗證檔案立即刪除，全程未寫入任何 Production 使用者資料。新增 `src/lib/holdingHistory.ts`（snapshot/entry 型別、entry 層級 fail-closed normalize、365 天 retention、same-day upsert、App-observed producer）並完成 `AppState`／`BackupPayload`／`normalizeState()`／`backupPayload()`／`stateFromBackup()` 接線；`calculateMetrics()` 新增 `quoteAvailable` 欄位；**Schema／Version：NO SCHEMA BUMP**；`SYNCABLE_TOP_LEVEL_FIELDS`／`netWorthHistory.ts`／`src/lib/**`（既有部分）／Worker／quote provider 均 0 diff；**本輪未接任何 Consumer UI**，`/assets` 既有畫面 0 diff。**UR-TODO-078 Phase A 正式 CLOSED／Production Verified；Phase B（per-holding／growth／defensive sparkline Consumer）維持 NOT STARTED／REQUIRES CONSUMER CONTRACT AUDIT，整體 UR-TODO-078 仍為 OPEN，不視為完全 CLOSED。** 詳見 `008_TODO_BACKLOG.md`（v2.6）／`012_AI_HANDOVER.md` UR-TODO-078 正式條目。

**（歷史記錄，已由上方 Final Closeout 條目取代）UR-TODO-078 Per-Holding Historical Snapshot Foundation Phase A — Implementation complete／Draft PR／Awaiting Preview acceptance（2026-08-23）。** Branch `feat/ur-todo-078-holding-history-foundation`（自 origin/main `adc6902` 開出）新增 `src/lib/holdingHistory.ts`（snapshot/entry 型別、normalize、producer、365 天 retention、same-day upsert、equality helper）並完成 `AppState`／`BackupPayload`／`normalizeState()`／`backupPayload()`／`stateFromBackup()` 接線與 `calculateMetrics()` 新增 `quoteAvailable` 欄位；本輪**未接任何 Consumer UI**，`/assets` 既有畫面 0 diff。開發中發現並修正一個真實 bug：producer effect 需比照既有 `dipAlerts` effect 模式，在呼叫 `setState()` **之前**做相等性判斷（而非在 updater 內部），因為 `setState()` 每次呼叫皆無條件 `normalizeState()`＋提交，已以 `git stash` A/B 測試驗證修正正確。另揭露一項**與本 Sprint 無關的既有 dev-server 問題**（本機 `dev` launch profile 全新/無 localStorage 時 console 出現 `Maximum update depth exceeded`，已確認 pristine origin/main 同樣重現），已建立獨立背景任務盤點，不佔用本 Todo 範圍。`npm run test:ur-todo-078`（38/38 pass）；`test:ci` chain 除既有已知 Windows CRLF/LF-only `clecTwReferenceHistoricalValidation.test.ts` 失敗外其餘全數 pass；TypeScript／Production build／Preview build／`git diff --check` 均通過；`npm audit` 4 項既有依賴漏洞與本 Sprint 無關（無新增依賴）。**尚未 Merge，尚未 Production 部署，等待使用者於 isolated Preview 驗收授權。** 詳見 `008_TODO_BACKLOG.md`（v2.5）／`012_AI_HANDOVER.md` UR-TODO-078 正式條目。

**（歷史記錄，已由上方 Implementation 條目取代）UR-TODO-078 Per-Holding Historical Snapshot Foundation — Final Contract Reconciliation，Phase A 正式判定 READY FOR DEVELOPMENT（2026-08-23）。** 對齊最新 origin/main `adc6902`（含 PR #424）後完成逐行 Persistence／Backup／Sync／Schema Wiring Audit，修正上一輪兩處錯誤（`quoteAvailable` 定義訂正為 `hasPreservedQuote || hasLatestPrice`；確認 `SYNCABLE_TOP_LEVEL_FIELDS` **不需要**加入 `holdingHistory`，因該陣列僅是已退役 Firebase `baselineFieldFingerprints` 的欄位 allowlist）。確認 `normalizeState()`／`backupPayload()`／`stateFromBackup()` 皆為顯式欄位列舉，`holdingHistory` 需明確接線（比照 `netWorthHistory` 既有模式，接線點已標明行號）；Schema／Version 最終判斷 **NO SCHEMA BUMP**（附四點實證）；365 天 retention 精確邊界公式沿用 `historyForRange()` 同一公式，消除 off-by-one。Phase A Development Readiness Gate 十項全數明確，**判定 READY FOR DEVELOPMENT**，尚未開始開發、未建功能 Branch，等待使用者下達「開始開發」。本輪僅修改 `AI_CONTEXT/**` 治理文件與重建 Bundle，`src/**`／`tests/**` 均 0 diff。詳見 `008_TODO_BACKLOG.md`（v2.5）／`012_AI_HANDOVER.md` UR-TODO-078 正式條目。

**（歷史記錄，已由上方 Final Contract Reconciliation 取代）新增 UR-TODO-078 Per-Holding Historical Snapshot Foundation（2026-08-23，OPEN／NEEDS CONTRACT AUDIT／PLANNED，純治理建檔，尚未開始開發）。** 為 UR-TODO-076 已明確拍板的「成長資產／防守資產／個股近 1 個月趨勢無逐日持久化資料 → fail-closed」缺口建立正式 history / persistence contract：一天一筆 `HoldingHistorySnapshot`／`HoldingHistoryEntry`（`symbol`／`shares`／`price`／`marketValue`／`assetClass`／`quoteAvailable`），沿用 `canonicalCalendarDay()`（Asia/Taipei）；App-observed producer（非背景排程、非交易所每日收盤），同日覆蓋、365 天 retention、禁止 historical backfill、closed/archived holding 歷史不刪除；Quote unavailable 日採方案 B（保留並標記 `quoteAvailable=false`，與既有 quote fallback／`*Available` 旗標慣例一致）；`AppState` 新增 additive `holdingHistory?`（不重用 `netWorthHistory`），判斷不需要 schema version bump；分 Phase A（Foundation，本輪僅規格）／Phase B（Asset Allocation sparkline consumer，獨立授權）。本輪僅修改 `AI_CONTEXT/**` 治理文件與重建 Bundle，`src/**`／`tests/**` 均 0 diff，未建立功能 Branch。詳見 `008_TODO_BACKLOG.md`／`012_AI_HANDOVER.md` UR-TODO-078 正式條目。
...
**UR-TODO-075 Holding Detail Information Architecture & Visual Refinement 已正式 CLOSED／Production Verified（2026-08-22）。** 使用者完成本機 Preview 人工驗收 PASS 後，PR [#412](https://github.com/hyc640110/family-universal-rebalance/pull/412) final head `202f96572d50e1c612854fa89c9ed6adda9b64de` 已由 `hyc640110` 於 `2026-08-22T11:48:49Z` 以一般 2-parent merge commit `696ea2f4d1d9b5ae8fafcc192046a00a2301cb37` 合併（parents `7e479636f7245b1cd0d8102db5a8b95f5822016d`／`202f96572d50e1c612854fa89c9ed6adda9b64de`；未使用 admin override，非 squash／非 rebase）。merge 後 main push 觸發之 Deploy GitHub Pages run [32571291838](https://github.com/hyc640110/family-universal-rebalance/actions/runs/32571291838) success，`event=push`、`headSha=696ea2f4d1d9b5ae8fafcc192046a00a2301cb37`，build／deploy 均成功。Production 唯讀驗證 PASS：HTTP 200、`environment=production`、base `/family-universal-rebalance/`、JS `/family-universal-rebalance/assets/index-CABEEP71.js` 與 CSS `/family-universal-rebalance/assets/index-BdoQJe4A.css` 均為 HTTP 200，未混入 `/preview/`。資產頁、持股列表 elevated gray surface、Holding Detail 新版 full-screen layout、Ring summary card、基本／損益／配置資訊、預設收合投資設定、資產管理與返回操作均正常；390×844 無 horizontal overflow，console 無 error，未寫入 Production 使用者資料。此 Sprint 僅變更 presentation／tests；`src/lib/**`、schema、persistence、localStorage／JSON Backup、Financial Event Ledger／attribution、Rebalance、AI Decision、CLEC、Household Liquidity、`environmentBoundary`、Preview Infrastructure 與 Production Deployment Policy 均未變更。PR #413 為獨立 Infrastructure work，已於後續治理 closeout 完成 runtime isolation verification，非本 Todo 的 blocking item。
...
經全站 dark surface design token 系統（page/surface/surface-2 層次、primary/secondary/muted 文字、primary/primary-hover、subtle border、card shadow/radius）、放大 Mobile-first typography、統一全站約 850 處 hardcoded 顏色為 token（涵蓋 Home／Assets／Analysis／Market／Tools／Settings／Portfolio-Risk 等頁）、Mobile Bottom Navigation 與 Desktop Sidebar active/inactive 視覺一致化、修正 allocation ring 因軌道對比不足而不可見的 root cause、Holding Card 詳細按鈕降為 Secondary Action，共四輪迭代開發與 Preview iPhone Safari 真機驗收（詳細逐輪內容見 `008_TODO_BACKLOG.md` UR-TODO-073 正式條目）。使用者於第四輪 Preview 驗收後決定：**上述範圍已足以結束 UR-TODO-073 Phase 1**，但 Holding Card 的 compact 資訊排版仍有精修空間，已明確 defer 至獨立的 **UR-TODO-074**（OPEN／PLANNED，尚未開始開發），**不視為本次 Production Verified 的一部分，亦不得因本次 Merge 而視為已開始**。`financial semantic diff = 0`、`persistence semantic diff = 0`；`src/lib/**`、schema、persistence、Navigation IA、UR-TODO-070／071／072 interaction contract 全程 0 diff（各輪皆有對應 regression test 鎖定並持續 pass）。台股「上漲＝紅、下跌＝綠」市場色彩契約全程未反轉。**本次結論不宣稱** Holding Card layout 已達最終設計——該項目正式移交 UR-TODO-074 持續精修。
...
開發過程中發現並經使用者明確授權修正 `src/lib/taiwanTradingCalendar.ts`：新增 `2025-09-29`（孔子誕辰紀念日／教師節，原訂 2025-09-28 適逢週日之官方補假）與 `2025-10-24`（臺灣光復暨金門古寧頭大捷紀念日，原訂 2025-10-25 適逢週六之官方補假）至 `CLOSED_DATES_BY_YEAR[2025]`，這是 **2025 年正式國定假日補假／市場休市的完整性修正**（非推測颱風假、非「多檔無資料即視為休市」的泛化 heuristic），已有 regression test 鎖定兩日為 `closed` 且相鄰交易日不受影響。
**Closure reason**：UR-TODO-014 的三項組成——Foundation（純 backtest library）、014-A（正式 TWSE 官方歷史資料來源與驗證）、014-A2（延伸真實歷史 full_rebalance 觸發時機驗證）——均已完成、測試通過並 Production Verified；原本因「real-history window 內四組 preset 皆未觸發 full_rebalance」而判定 EXTEND VALIDATION WINDOW 的缺口，已由 014-A2 正式補齊。UI 與 transaction-cost model 依 2026-08-20 Review Mode Contract Audit 之 CONDITIONAL 決策，經本次正式 Closeout Audit 確認移出 UR-TODO-014 closure requirement，**不再視為未完成項目**。**Remaining closure requirement = NONE。** 未來若需要 UI、transaction-cost model、slippage、next-day execution model，必須另立新的獨立 UR-TODO 並重新 Contract Audit，不得重新解釋為 UR-TODO-014 遺留缺口。詳見 `008_TODO_BACKLOG.md`／`012_AI_HANDOVER.md` 正式條目。

---

**（歷史記錄，已由上方最新正式條目取代）UR-TODO-014-A2（Extended Real-History Trigger Validation）開發完成，PR [#401](https://github.com/hyc640110/family-universal-rebalance/pull/401) 已開為 Draft，尚未 Merge。** Branch `feat/ur-todo-014-a2-real-history-trigger` 自 `origin/main` `2e1399ffaa6b4c41564e5a0ac46dc49a289358c7` 開出，同步至 final head `7abb630346b1ab11866aeab88f5d73ae6e1d5976`（此 SHA 為本次治理同步 commit 之前的 final head；本次治理同步 commit 本身造成的新 final head 落差留待下一次治理同步追平，避免無限自我更新循環）。延伸既有 UR-TODO-014-A validation-only reference fixture（prototype=0050、leveraged=00631L、cash-like=00865B，仍非 CLEC canonical mapping／Production 預設／AppState role mapping／使用者持股／UR-TODO-058）window 從 `2025-06-02～2025-08-29` 延伸至 `2025-06-02～2026-04-30`，同一 TWSE 官方 `STOCK_DAY_AVG`／`ETF 分配收益` 唯讀 pipeline，將 00631L 22:1 分割（原僅 standalone evidence）正式納入主 backtest window。**AC1 已滿足：找到真實歷史資料產生的 `full_rebalance` 觸發**——`clec-5050` 於 `2025-09-12`（target leveraged=50，drift 10.354pp ≥ threshold 5×2=10pp；前一日 2025-09-11 drift 9.818pp 仍為 `rebalance_consider`）；四組 preset 現皆至少觸發一次（442／433 於 2026-01-05；703 於 2025-10-07 與 2026-04-22；5050 於 2025-09-12 與 2026-01-27；原本皆為 0）。共同有效交易日 214（原 60）、period 213（原 59），fixture SHA-256 更新為 `5c143ec124492934e6e1dcb115b68b5d71414691e252d9440601164ce2221c8c`（原 `d9c049f2f5b4045244bfa0842eea41e878839d1dfad0a116f20bd0202f36f338`）。**開發過程中發現並經使用者授權修正 `src/lib/taiwanTradingCalendar.ts` 既有 2025 年曆缺口**：`2025-09-29`（教師節適逢週日之補假）與 `2025-10-24`（臺灣光復節適逢週六之補假）為官方國定假日補假，原清單缺漏，經使用者確認官方依據後授權新增，並補上 regression test；此為本 Sprint 額外必要修改，範圍嚴格限於這兩個已有正式依據的日期，不含任何「多檔同時缺資料即視為休市」的推論邏輯。`src/lib/clecHistoricalBacktest.ts`／`clecStrategyRules.ts`／`allocationPresets.ts` 三個 CLEC 核心檔案完全未修改，沿用既有 Production-verified Foundation。final head 的 CI Verification run [32379932816](https://github.com/hyc640110/family-universal-rebalance/actions/runs/32379932816) success：`npm run test:ur-todo-014`（CI 11/11 pass）、`test:ur-todo-014-a`（**CI 18/18 pass**）、`test:ci` 全鏈（CI 全數 pass、log 零筆 `##[error]`）、`npm run build`（含 `tsc -b`，CI production／preview 皆 build 成功）皆為 CI Linux／LF canonical checkout 上的實測結果（本機 Windows `core.autocrlf=true` checkout 因 CRLF 差異一度顯示 16/18～17/18，已確認純屬本機環境假性差異，非程式邏輯問題，以 CI 結果為準）。`git diff --check` 本機獨立重跑亦 pass（CI workflow 本身未將此列為獨立 step）。fixture canonical SHA-256（以 `git show` 取出的 git 實際儲存內容計算）：`5c143ec124492934e6e1dcb115b68b5d71414691e252d9440601164ce2221c8c`。**尚待 Preview／使用者驗收與 Merge 授權，AI 未自行 Merge。**

**UR-TODO-014-A（CLEC Taiwan Reference Historical Validation）已正式 MERGED／Production Verified，`origin/main` 正式基線更新為 `0341933a852076b5896d90f44fe351284954f942`。** PR [#399](https://github.com/hyc640110/family-universal-rebalance/pull/399) final head `f916bacc47dd3ff3865dbbf2da0d3dc65088d21d` 已由 `hyc640110` 於 2026-08-20T13:08:10Z 以一般 2-parent merge commit `0341933a852076b5896d90f44fe351284954f942` 合併（parents：`35bb19a758f886ae2de57fc55a1bc860c65c5798`／`f916bacc47dd3ff3865dbbf2da0d3dc65088d21d`；未使用 admin override）；merge 後 main push 觸發之 Deploy GitHub Pages run [32372567517](https://github.com/hyc640110/family-universal-rebalance/actions/runs/32372567517) success（`build` job 內含 `npm run test:ci` regression gate 通過，headSha 與 merge commit 一致），Production 已唯讀確認可正常載入。新增純資料準備 pipeline `scripts/clecTwReferenceDataset.ts`＋versioned offline fixture `tests/fixtures/clecTwReferenceV1.json`（`clec-tw-reference-v1`，SHA-256 `d9c049f2f5b4045244bfa0842eea41e878839d1dfad0a116f20bd0202f36f338`）＋ 15 項新測試 `npm run test:ur-todo-014-a`，重用既有 Production-verified `runClecHistoricalBacktest()`（未修改），對 442／433／703／5050 四組 CLEC preset 執行真實官方歷史資料驗證。Reference Validation Portfolio v1（明文標示 validation-only，非 CLEC canonical mapping／Production 預設組合／AppState role mapping／使用者持股 mapping／UR-TODO-058 契約）：prototype=0050、leveraged=00631L、cash-like=00865B；股價與配息資料唯一來源為 TWSE 官方 `STOCK_DAY_AVG`／`ETF 分配收益`，2025-06-02～2025-08-29（60 共同有效交易日）完整正規化 0050 官方 4:1 分割與除息（distribution-inclusive return）、獨立驗證 00631L 官方 22:1 分割證據（該分割發生於 window 之外，未納入主 backtest）。四組 preset 於此 3 個月 window 內 rebalanceCount 皆為 0（真實計算結果，未達 full_rebalance 門檻，非人為挑選）。`npm run test:ur-todo-014`（既有 11/11）、`test:ur-todo-014-a`（新增 15/15）、`test:ci`、`build` 全數通過。**UI 與 transaction-cost model 已依 2026-08-20 Review Mode Contract Audit 拍板移出 UR-TODO-014 closure requirement（CONDITIONAL）。UR-TODO-014-A Merge 完成 ≠ UR-TODO-014 CLOSED；整體 Todo 仍 OPEN**，原因是 real-history full_rebalance trigger timing 尚未由真實歷史資料覆蓋（現有 window 內四組 preset 皆未實際觸發），Closeout Audit（2026-08-20）判定為 **EXTEND VALIDATION WINDOW**，待 UR-TODO-014-A2（Extended Real-History Trigger Validation，PLANNED／NOT STARTED，本次治理同步僅建檔，未開始開發）補強。詳見 `008_TODO_BACKLOG.md` UR-TODO-014 正式條目。
...
以單一 ☰（`lucide-react` `Menu` icon，3 條水平線）drag handle 取代 UR-TODO-070 的 ↑/↓ 按鈕；`holdingDisplayOrder` 繼續作為唯一 display-order persistence，不重排 `AppState.holdings`、不 mutate `m.rows`、不影響 Rebalance／AI Decision／Financial Event Ledger／attribution／Firebase／財務公式。Native Pointer Events（Immediate Drag，無 long-press threshold，第一版不做 auto-scroll，未新增 drag-and-drop dependency）；Desktop（≥901px）handle 為整列最右側獨立操作區，Mobile（≤768px）與 identity 共用 header 第一列；Keyboard 沿用既有單步 `moveHoldingDisplayOrder()`，新增 `aria-live="polite"` 排序結果播報。

開發期間經歷 2 輪 iPhone Safari Preview 人工驗收：**Round 1** 發現「上→下長距離拖曳失敗」與「handle 圖示為六點 grip、不符合核准的 ☰」兩項 blocking issue，判定 FAIL／Refinement Required。Root cause 定位為：原設計採 element-level `setPointerCapture`，被 capture 的 handle 在每次 live-reorder 皆會被 React 重新定位到新的 array index（DOM 位置改變），Safari 相較 Chromium 更容易在元素被重新定位時自動釋放 pointer capture，導致長距離（多次重新定位）的向下拖曳中途失效，短距離／相鄰交換則較容易「運氣好」躲過此問題，與回報的方向性不對稱現象吻合。**Round 2** 修正為 `document` 層級的 `pointermove`／`pointerup`／`pointercancel` listener（drag 開始時掛上、結束／取消／unmount 時卸除），完全不依賴 element pointer capture；icon 由 `GripVertical` 改為 `Menu`。修正後 `npm run test:ur-todo-071`（41 tests）／既有 `test:ur-todo-070`（25 tests）／完整 `test:ci`／typecheck／build 皆 pass，iPhone Safari 真機重新驗收 PASS。

Final Review 記錄一項**已知低風險 follow-up observation**：`HoldingOrderHandle` 的 `handlePointerDown` 未防護「同一 handle 在前一次 drag 尚未 end/cancel 前又收到第二次 pointerdown」的情境（例如同一 handle 上的多點觸控），理論上可能造成該次 document listener 未被移除（記憶體洩漏，不影響排序資料正確性，因洩漏的 listener 僅回應已失效的舊 `pointerId`）。目前無 Production／iPhone 實際失敗證據，Round 2 真機驗收與 Production 唯讀驗證皆未觸發此情境；**目前無需另立新的 UR-TODO 編號**，後續若出現可重現問題再另行盤點。
...
**UR-TODO-023（月末交易對帳）已於 2026-08-18 CLOSED／Production Verified。** 使用者完成 Closeout Scope Review 後正式採納 **A. Direct Closeout**：023-A Monthly Transaction Reconciliation Preview 已完整滿足本 Todo 的 transaction-level 核心目的，**不建立 UR-TODO-023-B／023-C**。PR [#392](https://github.com/hyc640110/family-universal-rebalance/pull/392) final head `b41e0e5c5bf3b3fffaf5732a96e728d55e4c0cb2` 已由 `hyc640110` 於 2026-08-18T14:41:46Z 以一般 2-parent merge commit `a31ad5f9511d33f7b9226138fbb9c241b7636674` 合併（parents：`c10b26d9219cda6d50e799f08c7e47e15dd188be`／`b41e0e5c5bf3b3fffaf5732a96e728d55e4c0cb2`；未使用 admin override）。CI `32148181979`、相同 final head 的 Preview workflow_dispatch `32148383148` 與 push/main Deploy GitHub Pages `32149911373` 均 completed/success；最後者為 push/main、head 與 merge commit 一致，regression/test gate、Production build 與 Pages deploy 均 success。023-A 由有效 Statement 列推導期間、需使用者明確確認，僅比較同帳戶 posted／non-excluded／period-scoped App 交易；external ID 優先、fingerprint 次之，唯一高信心才 matched，ambiguity 保持 possible，並呈現 Statement-only／App-only／invalid、one-to-one 與 fail-closed 結果。Preview 人工驗收與 Production 唯讀驗證均 PASS，未建立或匯入 Production 測試交易、未修改 Production localStorage。結果只存在 Import Center transient React state，不寫入 Financial Event Ledger，也不建立 persistent reconciliation state。Review 已判定 balance reconciliation 與 history persistence 均非 correctness、auditability 或 workflow 必要缺口：Statement import 沒有 authoritative opening／closing／running balance，App account balance 可能是 manual 或 transaction-derived，Financial Event Ledger reconciliation 與 net-worth snapshot 皆不是 account-specific Statement balance；未來若需要 balance certification，必須是新的產品需求與資料契約。若未來需要可重現、長期稽核 history，須先定義 Statement source／hash、mapping、account、period、transaction identity／snapshot、algorithm version、invalidation semantics 與 retention policy；在此前持久化 summary 會造成 duplicated／stale state、migration、localStorage／JSON Backup 複雜度與 ambiguous SSOT。故 balance UI／flow、history persistence、schema bump、migration、Backup extension、Ledger wiring、debug／observability UI 皆為 **Not required for UR-TODO-023 closeout**，不得再解釋為本 Todo 的隱含未完成範圍。無 schema、persistence、AppState、JSON Backup、ImportSession、duplicate formula、transaction write path 或 UR-TODO-022-A／B／C 契約變更；`test:ci` 1,407 pass、TypeScript、Production／Preview build、`git diff --check` pass。`npm audit --omit=dev --audit-level=high` 的 4 個 high vulnerabilities 為既有相依問題，未由本 Sprint 引入。
...
**UR-TODO-022-A（Rule-Assisted Category Suggestion Foundation）已於 2026-08-18 CLOSED／Production Verified。** PR [#385](https://github.com/hyc640110/family-universal-rebalance/pull/385) merge commit `9628d8aed9a5875047eb86cdd98b28b7f580849b`、CI `32096747958`、Preview `32096761194`、Production `32097751304` 均為已驗證證據。022-A 的 deterministic high-confidence safe-whitelist suggestion、fail closed、Preview-only metadata 與 explicit apply contract 詳情保留於 Todo Backlog；fully automatic classification 屬 Deferred／Future Enhancement，不再是 UR-TODO-022 closure blocker。
...
**UR-TODO-069（退休規劃固定支出卡片精簡＋刪除圖示防誤觸）已於 2026-08-17 正式 CLOSED／Production Verified。** PR [#373](https://github.com/hyc640110/family-universal-rebalance/pull/373) 已以一般 2-parent merge commit `23416db7e575cbbac38abb67f3b72d94d9d28d74` 合併（parents：`87777766f9e2c37bcae0bad35194cc20444ab67a`／`56b3f551bccb22407cdbda4005246cf68f3c9abb`，`mergedAt: 2026-08-16T13:17:12Z`，`mergedBy: hyc640110`）；現行正式基線為 `origin/main` `1b47a5d44cc20abd13c1b70d486a4fed17313ca3`。PR verify [run 31948775856](https://github.com/hyc640110/family-universal-rebalance/actions/runs/31948775856) success；main Deploy GitHub Pages [run 31949386977](https://github.com/hyc640110/family-universal-rebalance/actions/runs/31949386977) completed/success，head 與 merge commit 一致。Production HTTP 200／`environment=production`，公開 CSS bundle 已確認於 `max-width:768px` 對 `.retirement-expense-enabled` 套用 `flex-direction:row; white-space:nowrap`；390px 與 Desktop 本機 Preview 驗收均通過，Desktop 不套用 mobile override。匯入與自訂項目維持共用既有 JSX／class；桌機、JSX、刪除確認／`removeItem()`、Cash Flow、`retirementPlan`、schema、persistence、JSON Backup、Ledger、attribution、Household Liquidity 與 Rebalance 均未變更；對應 regression test 已納入並持續通過。
...
#### 0. Implementation Summary（2026-08-23）
...
#### 3.1 Snapshot 結構（正式契約）

新增獨立型別（命名沿用既有 `netWorthHistory.ts` 慣例，新檔案 `src/lib/holdingHistory.ts`）：
...
export type HoldingHistoryEntry = {
  ...
  price: number;
  marketValue: number;
  assetClass: AssetClass; // 'growth' | 'defensive'，沿用 src/App.tsx 既有 AssetClass
  quoteAvailable: boolean; // 見 3.8
};
export type HoldingHistorySnapshot = {
  date: string; // canonical Asia/Taipei day，格式同 NetWorthSnapshot.date（YYYY-MM-DD）
  holdings: HoldingHistoryEntry[];
};
```

- 命名理由：`symbol`／`shares`／`assetClass` 直接沿用既有 `Holding` type（`src/App.tsx:135`）欄位名稱；`price`／`marketValue` 沿用既有 `calculateMetrics()` 內 `rows` 的欄位命名（`row.quote.price`／`row.marketValue`），不新創同義詞。
- 不重用 `Quote` 完整型別（`previousClose`／`change`／`changePct`／`volume` 等對「歷史市值走勢」consumer 無必要，且會顯著放大 365 天儲存量，見第 5 節）；僅取 `quoteAvailable` 作為最小 provenance metadata（見 3.8）。
- `HoldingHistoryEntry` 不含 `avgCost`／`cost`／`pnl`：這些屬於「使用者輸入 + 當下計算」欄位，非需要逐日保存的市場歷史事實，且 `avgCost` 可能事後被使用者編輯，若寫入歷史會造成歷史資料被回溯竄改的語意風險。未來若 Consumer 需要歷史損益，須另立獨立 Contract Audit（非本 Todo 範圍）。

#### 3.2 Date

沿用既有 `canonicalCalendarDay()`（`src/lib/calendarDay.ts:18`），固定 `Asia/Taipei`。不建立第二套日期邏輯，`HoldingHistorySnapshot.date` 格式與驗證規則與 `NetWorthSnapshot.date` 完全一致（`/^\d{4}-\d{2}-\d{2}$/`，並沿用 `netWorthSnapshotReadBoundary.ts` 的 `validDate()` UTC 往返驗證手法）。

#### 3.3 Producer

正式語意：**App-observed per-holding historical snapshot**（非背景排程、非交易所每日收盤歷史）。

- 寫入時機：App 成功取得有效報價後（沿用 `calculateMetrics()` 已計算出的 `rows`），依當下 `holdings`／`quote`／`marketValue` 產生今日 snapshot。
- 同日再次成功更新時，以最新一次合格計算結果覆蓋當日 snapshot（見 3.4）。
- **明確聲明**（必須寫入使用者可見文件／型別註解）：若使用者某天沒有開 App 或沒有成功更新報價，那一天可能沒有 snapshot；此非完整每日收盤資料，不得宣稱為交易所官方每日收盤歷史。
- Producer 函式建議命名 `holdingHistorySnapshotFromRows()`（沿用 `netWorthSnapshotFromTotals()` 命名慣例），輸入為 `calculateMetrics()` 的 `rows`（`{ symbol, shares, assetClass, quote: { price, error }, marketValue }[]`），輸出 `HoldingHistorySnapshot`。

#### 3.4 Same-day overwrite

一天只保留一個 snapshot；同日重複 producer 以最新一次合格計算結果整筆覆蓋當日 snapshot（非逐 holding merge，避免當日內某些 holding 用舊資料、某些用新資料的不一致狀態）。語意與 `upsertNetWorthSnapshot()`（`src/lib/netWorthHistory.ts:49`，`findIndex` 命中即整筆取代）一致，新增 `upsertHoldingHistorySnapshot()` 對應函式。

#### 3.5 Historical backfill

**禁止**：上線前不存在的 snapshot＝不存在，不得以「今日股數 × 舊價格」、交易推測或任何方式補值。此為本 Todo 最高優先的正式限制，違反即視為 Contract 違約。

#### 3.6 Closed / Archived holdings

既有歷史 snapshot 保留，不因 `shares=0`、`isArchived=true` 或 holding 從目前 active list 移除而追溯刪除。`HoldingHistoryEntry` 不記錄 `isArchived` 狀態本身（該狀態屬於「現在」的 holding 屬性，非歷史市值事實）；若未來 Consumer 需要區分「當時是否已封存」，須另行 Contract Audit。

#### 3.7 Retention（已授權的資料移除政策）

- 只保留最近 **365 天**（含今日，`canonicalCalendarDay()` 判定）。
- 超過 365 天的 snapshot 在新增／upsert 新 snapshot 時自動淘汰。
- Retention 由純函式處理，函式建議命名 `pruneHoldingHistoryRetention(rows, now)`（沿用 `historyForRange()`／`shiftCanonicalCalendarDay()` 既有 helper），內建於 `upsertHoldingHistorySnapshot()` 呼叫鏈中，不需額外手動觸發。
- **精確邊界公式（2026-08-23 Final Contract Reconciliation，逐行核對 `historyForRange()` 後明訂，消除 off-by-one 模糊）**：沿用 `src/lib/netWorthHistory.ts:50` 完全相同的公式——`cutoff = shiftCanonicalCalendarDay(canonicalCalendarDay(now), -364)`，保留規則為 `snapshot.date >= cutoff`。設 `today = canonicalCalendarDay(now)`，則保留範圍為 `[today 往前 364 天, today]`（含頭含尾），共計 **恰好 365 個 canonical calendar day**（Asia/Taipei）。第 365 天（即 `today - 364`）保留；第 366 天（`today - 365`）不滿足 `>= cutoff`，於 upsert 時淘汰。此公式與既有 `historyForRange(history, '1y', now)` 對 365 天 range 的既定語意完全一致，不建立第二套天數計算邏輯。
- 需要 boundary tests：恰好第 365 天保留、第 366 天淘汰、跨年／跨月邊界（例如 `today` 落在 2 月且回溯跨越閏年 2/29）、日期字串字典序排序與實際日曆序一致。

#### 3.8 Fail-closed（Consumer 規則，先寫入 Foundation 契約供未來遵守）

- 畫 trend 前至少需要 2 個有效歷史點；不足時顯示「資料不足」，不估算、不補線、不 forward-fill／backward-fill。
- **Quote unavailable 日的正式決定：方案 B（保留 entry 並標記 unavailable）**。
  - 理由（Contract Audit 結論）：既有 `calculateMetrics()` 對 quote error 已有成熟的 fallback 慣例——`hasPreservedQuote`／`hasLatestPrice` 判斷失敗時，仍以 `h.avgCost` 回退計算 `marketValue`（`src/App.tsx:549`），且 `netWorthHistory` 對應的 `totalAssetsAvailable`／`cashAvailable`／`netWorthAvailable` 也是「entry 永遠寫入、以 boolean 旗標標記可用性」而非「整日跳過」（`src/lib/netWorthHistory.ts:4`）。方案 B 與既有兩項慣例（quote fallback、availability 旗標）皆一致；方案 A（整日跳過該 holding）會在「使用者當天仍持有該 holding 但報價服務暫時失敗」時製造歷史資料的假性缺口，且與既有 `netWorthHistory` 「盡量保留可用近似值＋標記」的產品慣例矛盾。
  - **修正（2026-08-23 Final Contract Reconciliation，逐行核對 `calculateMetrics()` 後訂正）**：`quoteAvailable` 正式定義為 `hasPreservedQuote || hasLatestPrice`（`src/App.tsx:549` 內 `const price = hasPreservedQuote || hasLatestPrice ? num(q.price) : num(h.avgCost) || num(q.price)` 所用的同一條件），**非**僅 `hasLatestPrice`（上一輪初稿誤寫）。理由：`hasPreservedQuote`（`q.error` 存在但仍有合法時間戳與正值價格的「保留舊報價」情況）與 `hasLatestPrice`（本次成功取得的新報價）皆代表 `price` 為**真實市場觀測值**而非 `h.avgCost` 回退值；只有兩者皆為 false 時，`price` 才會回退為使用者的持股成本均價（非市場價格）。`quoteAvailable=false` 時 `price`／`marketValue` 仍寫入（沿用當下 fallback 計算結果，與 UI 現有顯示邏輯一致），但 Consumer 在計算 trend／sparkline 時**不得**將 `quoteAvailable=false` 的點視為與正常點等權重的真實市場資料點——正式規則留待 Consumer Sprint 的 Contract Audit 決定（例如是否整段標記為不連續），本 Foundation 僅負責保存原始事實與 availability 旗標，不做 Consumer 端判斷。
  - Phase A／Phase B 職責明確分離：**Phase A 只忠實保存** `price`／`marketValue`（無論 genuine 或 fallback）與 `quoteAvailable` 旗標，不做任何「是否可信」的過濾或轉換；**是否將 `quoteAvailable=false` 的點納入 sparkline／trend 計算是 Phase B 獨立、尚未拍板的決策**，留待 Consumer Sprint 開發前另行 Contract Audit（非本輪 Phase A blocking item）。

---

#### 4. Persistence Contract

**AppState**：新增 additive optional top-level 欄位 `holdingHistory?: HoldingHistorySnapshot[]`（`src/App.tsx:144`），與 `netWorthHistory?: NetWorthSnapshot[]` 同層級並存，**不覆寫、不重用** `netWorthHistory`（語意不同：一為 per-holding 市值，一為家庭總體淨值／現金）。

**Normalize**：新增獨立 read-boundary，沿用 `netWorthSnapshotNormalization.ts`／`netWorthSnapshotReadBoundary.ts` 兩層模式：
- `holdingHistoryNormalization.ts`（或併入 `holdingHistory.ts`）：malformed snapshot（`date` 非法或 `holdings` 非 array）整筆跳過；malformed entry（`symbol` 非字串、`shares`／`price`／`marketValue` 非 finite number、`assetClass` 不在 `'growth'|'defensive'`）**在該 snapshot 內跳過該筆 entry**（非整筆 snapshot fail-closed）——理由：與 `normalizeNetWorthHistory()` 的 row-level 處理粒度一致（該函式對單一 row 的欄位用 `n()` 寬鬆轉型而非整筆丟棄），且一個 snapshot 內某一檔股票的 malformed entry 不應連坐拖垮同日其他健康 entry 的資料可用性。
- Duplicate date：deterministic resolution 採「最後一筆勝出」（`selectLastOccurrenceByDate()`，與 `normalizeNetWorthHistory()` 一致）。
- Array safety：`Array.isArray()` 檢查，非 array 一律視為空歷史（沿用 `normalizeNetWorthHistory()` 慣例）。
- Finite number validation：沿用 `Number.isFinite()` 嚴格模式（不採 `netWorthHistory.ts` 的寬鬆 `Number()` 轉型，因為 per-holding 市值精度要求不應低於既有 `netWorthSnapshotNormalization.ts` 已建立的嚴格分類慣例）。

**localStorage**：**（2026-08-23 Final Contract Reconciliation，逐行核對後確認，非上一輪的類推）** `normalizeState()`（`src/App.tsx:415`）**不是**通用 spread-through——它以顯式欄位列舉的方式建構回傳物件（`normalizedCore` 明列欄位 + 結尾 `return { ...normalizedCore, ..., ...(netWorthHistory ? { netWorthHistory } : {}), fxRateHistory, syncMeta }`，`src/App.tsx:445/450`），**沒有**在結尾 `...s`（`s = {...defaultState, ...r}`）通盤回傳。因此新 top-level 欄位**不會**只因為加進 `AppState` type 就自動被 persist——必須明確在 `normalizeState()` 內解析 `r.holdingHistory` 並在回傳物件中顯式帶出（沿用 `netWorthHistory` 同一模式：`src/App.tsx:448` 解析＋`450` 條件式帶出）。寫入路徑為 `writeState()`（`src/App.tsx:477`：呼叫 `normalizeState()` → `stateWithPersistedFinancialEventLedger()`（spread 全部欄位，`452-458` 行，不需額外接線）→ `localStorage.setItem(STORAGE_KEY, ...)`）；讀取路徑為 `readStateWithSnapshotView()`（`src/App.tsx:461-476`：`localStorage.getItem(STORAGE_KEY)` → `JSON.parse` → `normalizeState()`）。`defaultState`（`src/App.tsx:282`）比照 `netWorthHistory` 前例**不需要**顯式 `holdingHistory` 預設值（optional 欄位，`undefined` 即合法初始狀態）。

**JSON Backup**：**（逐行核對確認）**
- `BackupPayload`（`src/App.tsx:145`）新增 `holdingHistory?: HoldingHistorySnapshot[]`。
- Export：`backupPayload()`（`src/App.tsx:491-498`）的 `payload` 物件為顯式欄位列舉，需新增 `...(normalized.holdingHistory ? { holdingHistory: normalized.holdingHistory } : {})`（`netWorthHistory` 同一位置的既有寫法，`src/App.tsx:498` 行內）。
- Import：`stateFromBackup()`（`src/App.tsx:505-529`）同樣為顯式欄位列舉的 `backupState` 物件，需新增 `...(r.netWorthHistory === undefined ? {} : { netWorthHistory: r.netWorthHistory })` 對應寫法（`src/App.tsx:524` 行內），即 `...(r.holdingHistory === undefined ? {} : { holdingHistory: r.holdingHistory })`。**實作後以整合測試核對確認的精確行為**（`backupState` 物件開頭為 `{ ...current, ... }`，見 `App.tsx:519`）：舊 Backup 缺欄位時該條件式不執行，`holdingHistory` 欄位**沿用 `...current` 帶入的匯入前本機既有值**（與 `netWorthHistory` 既有行為完全一致，非重置為 `undefined`）；只有匯入裝置本機也沒有既有值時，結果才維持 `undefined`（經 `normalizeState()` 正規化後等同 `[]`）。此為既有 `netWorthHistory` 慣例本身即如此（非本 Todo 新創），不產生 migration fake data。
- Export → Import → Re-export round-trip 必須可重現（新增對應測試）。

**Sync**：**（修正，逐行核對後推翻上一輪初稿）** `SYNCABLE_TOP_LEVEL_FIELDS`（`src/lib/syncState.ts:3`）**不需要**新增 `'holdingHistory'`。逐行核對確認：此陣列的唯一用途是 `sanitizeSyncFieldFingerprints()`（`syncState.ts:33-42`）內的欄位名稱 allowlist，用於過濾 `syncMeta.baselineFieldFingerprints`（`SyncMeta.baselineFieldFingerprints`，一個 Firebase 雲端同步時代用來做「逐欄位衝突偵測」的 fingerprint map，`syncState.ts:48`）——這是**已退役的 Firebase 雲端同步機制**（`016_Product_Decisions.md` §14：localStorage 為唯一 canonical runtime state）。`withoutRetiredCloudSyncMetadata()`（`syncState.ts:76-84`）在**每一個現行寫入路徑**（`writeState()`／`backupPayload()`，皆透過 `stateWithPersistedFinancialEventLedger()`／`backupPayload()` 內對 `syncMeta` 的呼叫）都會主動剔除 `baselineFieldFingerprints`，這條機制對未來寫入**永不再產生**，僅保留 `sanitizeSyncFieldFingerprints()` 做「讀到極舊 payload 時的相容性過濾」。`holdingHistory` 是全新欄位，不可能出現在任何舊 payload 的 `baselineFieldFingerprints` 裡，因此加入此陣列**沒有任何功能效果**。`holdingHistory` 本身是否被持久化，完全由上述 `normalizeState()`／`backupPayload()`／`stateFromBackup()` 顯式接線決定，與 `SYNCABLE_TOP_LEVEL_FIELDS` 無關。

**Schema／Version**：本輪判斷**不需要 schema version bump（NO SCHEMA BUMP，實證確認）**。實證：
1. `netWorthHistory?: NetWorthSnapshot[]` 加入時未新增任何 `*SchemaVersion` 欄位——現有 `AppState` 中僅 `accountSchemaVersion`／`cashAccountMigrationVersion`／`transactionSchemaVersion`／`financialEventSchemaVersion`／`importSchemaVersion` 五個版本欄位，皆對應各自獨立模組（`FinancialAccount`／`FinancialTransaction`／`FinancialEvent`／`ImportSession`），且都在該模組 shape 有 breaking change、需要一次性 migration 邏輯時才建立（例如 `CASH_ACCOUNT_MIGRATION_VERSION` 對應現金帳戶一次性遷移）。
2. `focusedSymbols`／`holdingDisplayOrder`／`cashFlowProfile`／`retirementPlan` 等其他 additive top-level 欄位，經 grep 確認**同樣沒有**任何 `xxxSchemaVersion` 配對欄位。
3. Bump 的觸發條件是「既有欄位 shape 變更、需要一次性 migration 把舊資料轉成新 shape」；`holdingHistory` 是全新欄位，沒有「舊資料」需要遷移——`undefined`／缺欄位本身就是合法的舊版本狀態，由 `normalizeState()`／`stateFromBackup()` 的 `undefined` 分支處理，不需要版本判斷式。
4. 結論：**A. NO SCHEMA BUMP**，與 `netWorthHistory` 等既有 additive optional 欄位前例完全一致。若未來 `HoldingHistoryEntry` shape 需要 breaking change（例如欄位改名或型別收斂），屆時才需要考慮新增獨立 `holdingHistorySchemaVersion`。

---

#### 5. Storage Growth（估算，非精確量測）

以 `HoldingHistoryEntry` JSON 序列化後單筆約 110～130 bytes（`symbol`／`shares`／`price`／`marketValue`／`assetClass`／`quoteAvailable` 六欄位）估算，365 天 retention：

| Holdings 數 | 總 entry 數（× 365） | 估算 JSON 大小 |
| --- | --- | --- |
| 10 | 3,650 | ≈ 400–470 KB |
| 20 | 7,300 | ≈ 800–950 KB |
| 50 | 18,250 | ≈ 2.0–2.4 MB |

- 相對於瀏覽器 localStorage 常見 5～10MB 上限，50 檔持股規模仍有餘裕，但需與既有 `netWorthHistory`、`transactions`、`financialEvents`、`accounts` 等既有欄位的**累加總量**一併考量（本輪未重新量測其他既有欄位的實際大小，僅盤點本 Todo 新增部分）。
- 365 天 retention 在目前家庭用戶規模（少量 holdings，個位數至十餘檔）下足以控制容量；若使用者持股數大幅超過 50 檔（超出目前產品定位假設），建議提出容量監控建議，本輪不自行改 persistence backend（例如換 IndexedDB），僅提出 contract 建議。
...
#### 7. Trend 計算語意（供 Phase B 遵循，本輪先行拍板）

- **Per-holding**：使用 `marketValue`（使用者實際持有部位的歷史市值走勢），不是單純股票價格走勢。
- **Growth／Defensive**：從每個 snapshot 保存的 `holdings[].assetClass`（**當日保存值**，非重新查詢今日 `normalizeAssetClass()`）聚合 `marketValue`。理由：使用者可能事後變更某檔標的的 `assetClass` 分類，若用「今天的分類」回頭解釋歷史，會讓歷史趨勢隨使用者設定變動而改寫，違反「歷史資料不可回溯竄改」原則（呼應 3.1 中 `avgCost` 不寫入歷史的同一理由）。

---

#### 8. Acceptance Criteria 草案（Phase A Foundation）

1. 同日多次更新只保留一筆 snapshot（整筆覆蓋，非逐 holding merge）。
2. 新一天成功 quote update 可新增 snapshot。
3. Quote unavailable 不製造假 `marketValue`（沿用既有 fallback 計算結果並以 `quoteAvailable=false` 標記，不偽造「正常報價」外觀）。
4. History ＞365 天會於 upsert 時自動淘汰。
5. 恰好 365 天 boundary 保留規則明確（boundary tests 覆蓋第 365／366 天）。
6. Archived／closed holding 舊歷史不被刪除。
7. 舊 localStorage 缺新欄位仍能正常開啟（normalize 後預設 `[]`）。
8. 舊 JSON Backup 缺新欄位仍能正常匯入（沿用匯入前本機既有值，本機也無既有值時 fallback＝`[]`，不產生 migration fake data，與 `netWorthHistory` 既有行為一致）。
9. Malformed snapshot／entry 依 4 節既定粒度 fail-closed（snapshot 層級與 entry 層級分別處理，不互相拖累）。
10. Export → Import → Re-export round-trip 保留 `holdingHistory`。
11. Mobile／Desktop consumer（Phase B）共用同一 `holdingHistory` SSOT（本輪先確認 Foundation 不做任何 platform-specific 分支）。
12. （Phase B）consumer points ＜2 時顯示「資料不足」。
13. 不存在 historical backfill／fake sparkline（Phase A 與 Phase B 皆適用，違反即視為 Contract 違約）。
14. 不修改 Rebalance／Household Liquidity／Risk／AI／CLEC／Ledger／attribution 語意（`src/lib/**` 既有模組除新增 `holdingHistory.ts` 相關檔案外，其餘 0 semantic diff）。

---

#### 8.5 Phase A Development Readiness Gate（2026-08-23 Final Contract Reconciliation）

逐項判定：

| 項目 | 狀態 |
| --- | --- |
| Snapshot contract | ✅ 明確（3.1／3.2） |
| Producer contract | ✅ 明確（3.3／3.4） |
| Availability write contract | ✅ 明確（3.8，`quoteAvailable = hasPreservedQuote \|\| hasLatestPrice`，已逐行核對訂正） |
| Normalize contract | ✅ 明確（4 節，snapshot 層級／entry 層級 fail-closed 粒度已定） |
| Retention contract | ✅ 明確（3.7，精確邊界公式已消除 off-by-one） |
| Persistence wiring | ✅ 明確（4 節，`normalizeState()`／`writeState()`／`readStateWithSnapshotView()` 逐行核對，需顯式接線，接線點已標明行號） |
| Backup wiring | ✅ 明確（4 節，`backupPayload()`／`stateFromBackup()` 逐行核對，接線點已標明行號） |
| Sync wiring | ✅ 明確（4 節，確認**不需要**改動 `SYNCABLE_TOP_LEVEL_FIELDS`，已推翻上一輪錯誤結論並附實證） |
| Schema decision | ✅ 明確（4 節，NO SCHEMA BUMP，已附四點實證） |
| Storage sizing | ✅ 明確（5 節，估算範圍不受本輪修正影響） |

**判定：UR-TODO-078 Phase A = READY FOR DEVELOPMENT。**

**非 Phase A blocking 的已知待辦（留待 Phase B Consumer Contract Audit，不阻擋 Phase A 開發啟動）**：
1. `quoteAvailable=false` 資料點是否／如何進入 Phase B sparkline／trend 計算（例如是否整段標記不連續）——3.8 已明訂 Phase A 僅負責忠實保存，此為 Consumer 端獨立決策。
2. Phase B 啟動時機（需 Phase A 上線後累積足夠天數的真實 snapshot 才有意義驗收）——6 節已載明，非 correctness 缺口，純屬 App-observed 語意的時間限制。

---

#### 9. 明確不包含（v1／Phase A 與 Phase B 皆不含）

Server-side scheduler、background daily job、真正交易所 daily close history、歷史股價 provider、transaction replay engine、corporate-action replay、split／dividend reconstruction、365 天以前資料 backfill、cloud database migration、Firebase revival、auto trading、Rebalance formula change。

---

#### 10. 依賴與邊界

- 依賴：UR-TODO-076（已 CLOSED／Production Verified，已明確拍板 fail-closed 決策與 `calculateMetrics()`／`normalizeAssetClass()` 既有 SSOT，本 Todo 沿用不重建）。
- 正式邊界：`AssetAllocationOverview`／`AssetAllocationDonutPanel`／`AssetOverviewCard`／`AssetAllocationDetailTable`／`MiniSparkline`（UR-TODO-076 既有元件）、`allocationColor()`／`FIXED_ALLOCATION_COLORS`／`ALLOCATION_COLORS` 色彩 SSOT、Rebalance／Household Liquidity／Risk／AI／CLEC／Simulator／Financial Event Ledger／attribution／Navigation IA 均不在本 Todo 修改範圍內（Phase A 完全不觸碰，Phase B 僅新增 sparkline 呈現，不修改上述既有語意）。
- 下一步：等待使用者明確下達「開始開發」後才進入 Phase A 唯讀盤點與 implementation；本輪為純規格建檔，未建立功能 Branch。
...
- Production 證據：main push 自動觸發 Deploy GitHub Pages run [32866817283](https://github.com/hyc640110/family-universal-rebalance/actions/runs/32866817283) success（event `push`、headSha 一致、main regression gate／Production build／Pages deploy success）；artifact-expired annotation 已由既有 fallback 正常處理，非失敗。Production HTTP 200。唯讀 DOM／computed-layout 驗證首頁及 `#/assets`：Desktop 與 320／390／430px 正常、無 horizontal overflow、Mobile detail table 不 render、無 Bottom Sheet／Modal、console 0 error；未建立假資料、未寫入 localStorage 或使用者資料。
- Consumer contract：Total Assets／Cash 仍使用 `netWorthHistory`；Growth／Defensive／per-holding 使用真實 App-observed `holdingHistory`。`quoteAvailable===true` 才能成為 trend point；false 的 avgCost fallback 完全排除。per-holding 用 snapshot `marketValue`，保留有效 `shares=0` 的 0；分類使用 snapshot 當日 `assetClass`；class 任一 entry unavailable 即整日 invalid，空集合為真實 0；30 天 window 缺日不補。0／1 valid point 顯示「近1個月趨勢資料不足」為正確 fail-closed；現有 Production 真實資料正屬此狀態，未人工注入。2+ valid points correctness 已由同一 final-head deterministic tests 證明。
- Phase A 與財務語意邊界：Phase B 是 historical read consumer。Phase A producer、persistence、Backup、365-day retention、schema、same-day overwrite、quoteAvailable producer semantics 皆 0 diff；calculateMetrics 財務公式、current allocation、target、deviation、Rebalance、Household Liquidity、Risk、AI Decision、CLEC、Simulator、Financial Event Ledger、attribution、quote provider、Worker 均 0 semantic diff。完整契約與驗收記錄見 `008_TODO_BACKLOG.md` §0.2。
...
- Production 證據：main push 自動觸發 Deploy GitHub Pages run [32652884649](https://github.com/hyc640110/family-universal-rebalance/actions/runs/32652884649) success（`build`／`deploy` 皆 success，`build` 內 `npm run test:ci` regression gate 通過；run annotations 中的 artifact-expired 訊息為既有 fallback 機制正常觸發，非部署失敗）；Production HTTP 200、deployment sha 與 merge commit 一致（`environment=github-pages`）、JS `index-ZiKPt2JD.js`／CSS `index-hMnE973C.css`（與本機最終 build 產物一致）。以**全新瀏覽器分頁**（無殘留 console 歷史）唯讀驗證：首頁完整渲染、`#/assets` 頁「資產配置」四張卡片皆維持 UR-TODO-076 既有 fail-closed「近1個月趨勢資料不足」，**Consumer sparkline 未出現**、持股卡片與報價正常、股價更新成功（4/4）、375px 無 horizontal overflow、**console 0 error**（先前另一分頁殘留的 404 經全新分頁重測排除，證實非本次 Production 產生）。額外唯讀下載 Production JS bundle 確認 `holdingHistory`（12 次）／`quoteAvailable`（10 次）欄位名稱字面存在於編譯產物中，驗證檔案立即刪除；驗證全程未寫入任何 Production 使用者資料、未建立測試持股、未修改設定。
- **最終 Phase A Contract（下一位 AI／開發者必須遵守）**：`HoldingHistorySnapshot{date, holdings: HoldingHistoryEntry[]}`／`HoldingHistoryEntry{symbol, shares, price, marketValue, assetClass, quoteAvailable}`；日期沿用 `canonicalCalendarDay()`（Asia/Taipei）；producer 為 App-observed（`src/App.tsx` 的 `currentHoldingHistorySnapshot` useMemo ＋ producer useEffect，沿用 `calculateMetrics()` 既有 `m.rows`，`hasUpdatedQuotes` 觸發，非背景排程）；同日整筆覆蓋（`upsertHoldingHistorySnapshot()`）；365 天 retention（`pruneHoldingHistoryRetention()`，公式與 `historyForRange()` 一致）；`quoteAvailable = hasPreservedQuote || hasLatestPrice`；`AppState`／`BackupPayload` 新增 additive `holdingHistory?`；`normalizeState()`／`backupPayload()`／`stateFromBackup()` 已接線；**NO SCHEMA BUMP**；`SYNCABLE_TOP_LEVEL_FIELDS` 0 diff。
- 邊界與下一步：`src/lib/**`（既有部分）、Rebalance、Household Liquidity、Risk、AI Decision、CLEC、Simulator、Financial Event Ledger、attribution、quote provider、Worker、`netWorthHistory.ts`、`syncState.ts` 均為 0 semantic diff。**Phase A 已 CLOSED，不得再修改**除非使用者提出新需求（視為新 Todo）。**Phase B（per-holding／growth／defensive sparkline Consumer）尚未開始**，啟動前必須：(1) 使用者明確下達「開始開發」；(2) Production 已累積足夠天數真實 snapshot；(3) 完成 `quoteAvailable=false` 是否／如何進入 trend 計算的獨立 Consumer Contract Audit（Phase A 僅忠實保存，未定義此規則）。另有一項與本 Sprint 無關的既有 dev-server 問題（本機 `dev` launch profile 全新/無 localStorage 時的 `Maximum update depth exceeded`）已建立獨立背景任務盤點（`task_c2a68a99`），不佔用本 Todo。

## 前一交接快照：UR-TODO-078 Phase A Implementation — Draft PR／Awaiting Preview Acceptance（2026-08-23）
...
- Production zero-impact：該 merge 的既有 Deploy GitHub Pages run [32573997202](https://github.com/hyc640110/family-universal-rebalance/actions/runs/32573997202) `push/main` success。Production HTTP 200、`environment=production`、base `/family-universal-rebalance/`、JS `index-CABEEP71.js`、CSS `index-BdoQJe4A.css` 與 merge 前完全一致；Preview runtime／Pages 啟用沒有產生新的 Production deployment。
- Isolated runtime proof：Deploy Isolated Preview run [32574245054](https://github.com/hyc640110/family-universal-rebalance/actions/runs/32574245054) success，requested／resolved source 均為 non-main `202f96572d50e1c612854fa89c9ed6adda9b64de`（當時 main `e24b3d0bf60078d7e62e0fc88e347bc347add5e3`），僅寫入 `hyc640110/family-universal-rebalance-preview` 的 `preview` branch（deployment commit `cdea4e4576bd325cda7590b49cb0517b43a99b78`）。
- 正式 Preview 架構：`family-universal-rebalance` → **Deploy Isolated Preview** → `family-universal-rebalance-preview`／`preview` → GitHub Pages [https://hyc640110.github.io/family-universal-rebalance-preview/](https://hyc640110.github.io/family-universal-rebalance-preview/)；Production 維持 [https://hyc640110.github.io/family-universal-rebalance/](https://hyc640110.github.io/family-universal-rebalance/)。兩者為獨立 Pages site。Preview Pages publishing source = `preview`／`/(root)`，遠端 Preview HTTP 200、`environment=preview`、base `/family-universal-rebalance-preview/`、390×844 無 horizontal overflow、Bottom Navigation／Assets／Holding Detail／console 均 PASS。
- 下一位 AI 的操作邊界：新功能 Preview 驗收優先走此 isolated path；必須提供非 main branch／commit／SHA，保留 workflow 對 main 字串與 resolved main SHA 的 fail-closed 防呆；Preview target 僅可為 `hyc640110/family-universal-rebalance-preview` 的 `preview` branch。不得把舊 Production Pages combined `/preview/` 當作標準 Preview，且不得因 Preview deployment 觸發 Production deployment。此輪只記錄既有 verified infrastructure，未修改 workflow／runtime code／Production Pages。
...
- Scope boundary：只包含 Holding Detail presentation／responsive behavior、持股列表 surface、必要 regression tests 與 script。`src/lib/**`、schema、persistence、localStorage／JSON Backup、Ledger／attribution、Rebalance、AI Decision、CLEC、Household Liquidity、`environmentBoundary`、Preview Infrastructure、Production Deployment Policy 均未變更。PR #413 是獨立 Infrastructure work，已於後續完成 MERGED／Infrastructure Installed／Isolated Remote Preview Verified，且從來不是本 Todo 的阻擋。
...
- Production facts：merge 後 main push 觸發之 Deploy GitHub Pages run [32383019454](https://github.com/hyc640110/family-universal-rebalance/actions/runs/32383019454) success（`build` job：`Run CI regression test gate (main)` 執行 `npm run test:ci` 全鏈通過、`Build production Vite app (main)` 成功；`deploy` job success）。Production 唯讀確認 HTTP 200、`environment=production`、bundle 為 `index-BZLdP2-T.js`／`index-DTD1MPZn.css`（與本次 build 產物一致，`Last-Modified` 對應本次 deploy 時間）。
- Real trigger evidence（AC1/AC2/AC3，Production Verified）：延伸真實歷史 window `2025-06-02～2026-04-30`（214 個共同有效交易日、213 periods），validation-only mapping 維持 prototype=0050／leveraged=00631L／cash-like=00865B，fixture `datasetVersion=2.0.0`，canonical SHA-256（git 實際儲存內容）`5c143ec124492934e6e1dcb115b68b5d71414691e252d9440601164ce2221c8c`。`clec-5050` 於 `2025-09-12` 產生真實 `full_rebalance`（`2025-09-11` maxDrift≈9.818pp／`rebalance_consider`／未 rebalance；`2025-09-12` maxDrift≈10.354pp ≥ threshold 10pp／`full_rebalance`／已 rebalance，trigger 後 portfolio 精確 reset 至 target weights）；四組 preset 皆至少觸發一次：442／433 於 `2026-01-05`；703 於 `2025-10-07`／`2026-04-22`；5050 於 `2025-09-12`／`2026-01-27`。`rebalance_consider` 全程未被誤算為交易。`src/lib/clecHistoricalBacktest.ts`／`clecStrategyRules.ts`／`allocationPresets.ts` 三個 CLEC 核心檔案於 Foundation／014-A／014-A2 全程完全未修改，沿用既有 Production-verified Foundation。
- Calendar correction（本 Sprint 額外必要修改，已授權）：`src/lib/taiwanTradingCalendar.ts` 新增 `2025-09-29`（孔子誕辰紀念日／教師節，2025-09-28 適逢週日之官方補假）與 `2025-10-24`（臺灣光復暨金門古寧頭大捷紀念日，2025-10-25 適逢週六之官方補假），屬 **2025 年正式國定假日補假／市場休市的完整性修正**，非推測颱風假、非「多檔無資料即視為休市」的泛化 heuristic；`tests/v6QuoteRefreshFreshnessConsistency.test.ts` 已鎖定兩日為 `closed` 且相鄰交易日（09-26／09-30／10-23／10-27）不受影響。
- Closure reason：Foundation（純 backtest library）、014-A（正式 TWSE 官方歷史資料來源與驗證）、014-A2（延伸真實歷史 full_rebalance 觸發時機驗證）三項組成均已完成、測試通過並 Production Verified。UI 與 transaction-cost model 依 2026-08-20 Review Mode Contract Audit 之 CONDITIONAL 決策，經本次正式 Closeout Audit 確認**不屬於**closure requirement。
- Scope Guard（PR #401 全程未觸碰）：UI、route、AppState、localStorage、persistence、JSON Backup schema、Financial Event Ledger、attribution、Production CLEC symbol mapping、Production Rebalance、Household Liquidity、historical Worker endpoint、runtime historical provider、UR-TODO-058、transaction-cost／slippage／next-day execution model。
- 下一位 AI／使用者：**UR-TODO-014 已無任何未完成範圍，不得再視為 active、blocker 或下一 Sprint candidate。** 未來若使用者提出 UI、transaction-cost model、slippage、next-day execution model 等需求，必須視為全新需求、另立獨立新 UR-TODO 並重新進行 Contract Audit，**不得**重新解釋為 UR-TODO-014 遺留缺口或自動延續舊編號。不得未經使用者要求就自行擴大範圍或搶先開始新 Sprint。

## 前一交接快照：UR-TODO-014-A2 Extended Real-History Trigger Validation（開發完成／PR Draft 待驗收，2026-08-20；UR-TODO-014 overall 仍 OPEN，歷史快照，已由上方最新正式條目取代）

- 狀態：**UR-TODO-014-A2 開發完成，PR [#401](https://github.com/hyc640110/family-universal-rebalance/pull/401) 已開為 Draft，尚未 Merge，尚未 Preview 人工驗收。UR-TODO-014-A2 完成 ≠ UR-TODO-014 CLOSED。整體 UR-TODO-014 仍 OPEN，需在 A2 Merge 後另立正式 Closeout Audit 才能判定。**
- Branch／PR facts：Branch `feat/ur-todo-014-a2-real-history-trigger` 自 `origin/main` `2e1399ffaa6b4c41564e5a0ac46dc49a289358c7`（PR #400 governance closeout merge）開出，同步至 final head `7abb630346b1ab11866aeab88f5d73ae6e1d5976`（本次治理同步 commit 之前的 final head；本次治理同步 commit 自身造成的新 final head 落差留待下一次治理同步追平，避免無限自我更新循環）。final head 的 CI Verification run [32379932816](https://github.com/hyc640110/family-universal-rebalance/actions/runs/32379932816) success：`test:ur-todo-014-a` **CI 18/18 pass**（本機 Windows CRLF checkout 曾顯示 16/18，已確認為純環境差異，見下方驗證段落）。PR #401 為 Draft，未使用 admin override，AI 未自行 Merge。
- 任務背景：UR-TODO-014-A 的 2026-08-20 Closeout Audit 判定 **EXTEND VALIDATION WINDOW**——既有 3 個月 window 內四組 preset `rebalanceCount` 皆為 0，只驗證 no-trigger（hold）路徑，未驗證 `full_rebalance` 真實觸發時機。使用者以完整 acceptance criteria（AC1~AC7）與 TDD 流程明確授權開始 A2 開發。
- 已完成契約：延伸既有 validation-only Reference Portfolio（prototype=0050、leveraged=00631L、cash-like=00865B，邊界與 014-A 完全相同，**不得**升格為 CLEC canonical mapping／Production 預設／AppState role mapping／使用者持股／UR-TODO-058）的真實歷史 window，從 `2025-06-02～2025-08-29` 延伸至 `2025-06-02～2026-04-30`，沿用同一 TWSE 官方 `STOCK_DAY_AVG`（`https://www.twse.com.tw/exchangeReport/STOCK_DAY_AVG?...`）／`ETF 分配收益` 唯讀 Browser pane pipeline，**未新增資料來源**。共同有效交易日 60→214、period 59→213；fixture `datasetVersion` 1.0.0→2.0.0，SHA-256 `d9c049f2f5b4045244bfa0842eea41e878839d1dfad0a116f20bd0202f36f338` → `5c143ec124492934e6e1dcb115b68b5d71414691e252d9440601164ce2221c8c`。
- **AC1（真實 full_rebalance trigger）已滿足**：`clec-5050`（target leveraged=50、cash-like=50，threshold drift=5×significantMultiplier=2=10pp）於 `2025-09-12` 產生真實 `recommendedAction === 'full_rebalance'`（weightsBeforeRebalance.leveraged=60.354%，drift 10.354pp ≥ 10pp）。**AC2（trigger boundary）已鎖定**：前一有效觀測日 `2025-09-11` drift 9.818pp（< 10pp，仍為 `rebalance_consider`，未觸發），已寫成 regression test（`tests/clecTwReferenceHistoricalValidation.test.ts` test `M`）明確斷言 trigger 日期、trigger 前一日狀態與 threshold 交叉條件，非僅斷言 `rebalanceCount > 0`。**AC3（語意）已驗證**：全程 `rebalance_consider` 觀測皆 `rebalanced: false`，未被計為 rebalance event。**AC5（既有 preset 不 regression）已驗證**：四組 preset 於延伸 window 內 rebalanceCount 皆 ≥1（442：1 次於 2026-01-05；433：1 次於 2026-01-05；703：2 次於 2025-10-07／2026-04-22；5050：2 次於 2025-09-12／2026-01-27），較 014-A 全 0 結果明確補上缺口，且未破壞既有計算路徑。**AC6（既有 normalization 未破壞）已驗證**：0050 4:1 split／distribution-inclusive return、00631L 22:1 split（本次新增 embedded 版本測試 `H2`）、common-date alignment 皆有對應 regression test 通過。**AC7（fixture hash 重新計算）已完成**：新 hash 已如上，未沿用舊 hash。
- 技術實作：`scripts/clecTwReferenceDataset.ts` 的 `deriveClecTwReferencePeriods()` 由「僅處理 0050 分割停市」泛化為 `splitBySymbol: Partial<Record<...>>` 處理任一參考標的的分割停市（`splitAdjustedPriceMap()` 現同時套用於 0050 與 00631L），讓 00631L 22:1 分割正式納入主 backtest window（split-adjusted，split 當日 leveraged period return 為 -4.38pp，naive unadjusted 約 -95.65%，兩者差距 >80pp 已測試）。`ClecTwReferenceFixture` 型別的 `'00631L'` 分支新增可選 `split?: ClecTwReferenceSplit` 欄位。0050 新增第二筆 in-window 除息（2026-01-22，每受益權 1.0 元，distribution-inclusive return，已測試 `I2`）；00631L／00865B 全歷史配息（2010-01-01～2026-08-20 查詢範圍）仍為 0 筆，直接沿用既有 provenance，未重查。**`src/lib/clecHistoricalBacktest.ts`／`clecStrategyRules.ts`／`allocationPresets.ts` 三個 CLEC 核心檔案完全未修改**，全程重用既有 Production-verified Foundation。
- **開發中發現並經使用者明確授權修正 `src/lib/taiwanTradingCalendar.ts`（本 Sprint額外必要修改，超出原始三檔案範圍）**：延伸資料時發現 `2025-09-29`／`2025-10-24` 三檔參考標的（0050／00631L／00865B）同時無官方收盤資料，AI 依既有 fail-closed 政策先停止並回報為 blocker（未自行假設為颱風假或逕行修改共用日曆檔）。使用者確認並提供正式依據：`2025-09-29` 為孔子誕辰紀念日／教師節（原訂 2025-09-28 適逢週日）之補假，`2025-10-24` 為臺灣光復暨金門古寧頭大捷紀念日（原訂 2025-10-25 適逢週六）之補假；使用者明確授權範圍：只新增這兩個已有正式依據的日期至 `CLOSED_DATES_BY_YEAR[2025]`，**不得**新增任何「多檔同時缺資料即視為休市」的泛化推論規則、**不得**新增颱風假推論機制、不得更動其他年份或既有 `taiwanTradingCalendarStatus()` 判定語意。provenance 已記錄為「2025 年政府修正版國定假日／補假」，明確**不是**颱風假。已於 `tests/v6QuoteRefreshFreshnessConsistency.test.ts` 新增 regression test 確認兩日為 `closed`、且緊鄰的正常交易日（09-26／09-30／10-23／10-27）不受影響。`quoteMath.ts` 是此檔案唯一其他消費端（既有 00631L／00865B 昨收價防呆機制），已重新執行相關測試（`quoteMath.test.ts`／`dipAlertsAfterQuoteUpdate.test.ts`／`aiDecision.test.ts`，共 30 tests）確認無 regression。
- 驗證結果（final head `7abb630346b1ab11866aeab88f5d73ae6e1d5976` 的 CI Verification run [32379932816](https://github.com/hyc640110/family-universal-rebalance/actions/runs/32379932816) success，為正式依據）：`npm run test:ur-todo-014`（CI 11/11 pass，Foundation 未改動）、`npm run test:ur-todo-014-a`（**CI 18/18 pass**）、`test:ci` 全鏈（CI 全數 pass，log 零筆 `##[error]`，含 `test:ur-todo-022-a`、firebase retirement regression、`test:risk-presentation`、`test:ci:unit-mjs`、`test:ci:checks`、全部 attribution／loan／fx／runtime-attribution 系列、`test:ur-todo-016`／`070`／`071`、`pretest:ci`／`posttest:ci`，共 1,089+ 測試無 fail）、`npm run build`（含 `tsc -b`，CI production／preview 皆 build 成功）。本機 Windows `core.autocrlf=true` checkout 曾因 CRLF 差異對 test C（fixture hash）與 test K（regex 比對）顯示 2 項假性失敗（16/18）——已用 `git show HEAD:` 取出 git 實際儲存的 LF 內容驗證能通過同一斷言／同一正則，並用 `git stash` 驗證 test K 的問題在 A2 任何改動前、對完全未修改的原始檔案就已存在，確認純屬本機環境差異、非本次改動造成的 regression。`npx tsc -b`：本機環境原本缺少 `pdfjs-dist`（`package.json` 已宣告但 `node_modules` 未安裝，既有環境缺口非本次改動造成），已執行 `npm install` 補齊後本機獨立重跑乾淨通過。`git diff --check`：本機獨立重跑 pass，無 whitespace 錯誤（CI workflow 本身未將此列為獨立 step）。Fixture canonical SHA-256（以 git 實際儲存內容計算）：`5c143ec124492934e6e1dcb115b68b5d71414691e252d9440601164ce2221c8c`。
- 明確不包含（Scope Guard，PR #401 未觸碰）：UI、route、AppState、localStorage、persistence、JSON Backup schema、Financial Event Ledger、attribution、Production CLEC symbol mapping、Production Rebalance、Household Liquidity、historical Worker endpoint、runtime historical provider、UR-TODO-058、transaction-cost／slippage／next-day execution model。
- 下一位 AI／使用者：**PR #401 為 Draft，尚未 Preview／人工驗收，AI 不得自行 Merge 或轉 Ready for review。** 待使用者完成驗收並明確授權後才可繼續 PR 流程。若使用者要求對 UR-TODO-014 做整體 Closeout Audit，須等 A2 實際 Merge 之後才有意義；A2 尚未 Merge 前，UR-TODO-014 整體維持 OPEN，不得標記為 CLOSED 或準備關閉。

## 前一交接快照：UR-TODO-014-A CLEC Taiwan Reference Historical Validation（MERGED／Production Verified，2026-08-20；UR-TODO-014 overall 仍 OPEN，待 UR-TODO-014-A2）

- 狀態：**UR-TODO-014-A 已正式 Merge 並 Production Verified。UR-TODO-014-A ≠ UR-TODO-014 CLOSED。整體 UR-TODO-014 仍 OPEN。**
- Merge facts：PR [#399](https://github.com/hyc640110/family-universal-rebalance/pull/399)，final head `f916bacc47dd3ff3865dbbf2da0d3dc65088d21d`，merge commit `0341933a852076b5896d90f44fe351284954f942`（parents `35bb19a758f886ae2de57fc55a1bc860c65c5798`／`f916bacc47dd3ff3865dbbf2da0d3dc65088d21d`；一般 2-parent merge commit，未使用 admin override），`mergedAt: 2026-08-20T13:08:10Z`，`mergedBy: hyc640110`。Merge 後 main push 觸發之 Deploy GitHub Pages run [32372567517](https://github.com/hyc640110/family-universal-rebalance/actions/runs/32372567517) success（`build` job 內 `npm run test:ci` regression gate 通過），Production 已唯讀確認可正常載入。`origin/main` 現行正式基線：`0341933a852076b5896d90f44fe351284954f942`。
- **Closeout Audit 判定（2026-08-20，Review Mode）：EXTEND VALIDATION WINDOW。** 逐項稽核 014-A 工程結果全數 PASS（見下方細節），但現有 real-history window 內四組 preset 的 `rebalanceCount` 皆為 0，只驗證了 no-trigger（hold）路徑；2026-08-16 範圍描述明文「驗證規則本身的歷史績效**與觸發時機**」，因此判定不足以視為 UR-TODO-014 整體收斂，需要下一個 bounded follow-up：**UR-TODO-014-A2（Extended Real-History Trigger Validation）**，狀態 **PLANNED／NOT STARTED**（本輪僅治理建檔，未開始開發，未建立 branch）。
- 新增檔案（PR #399，已 merge）：`scripts/clecTwReferenceDataset.ts`（純資料準備 pipeline：`deriveClecTwReferencePeriods()`／`runClecTwReferenceValidation()`／`computeDatasetHash()`；放在 `scripts/` 而非 `src/lib/` 是因為使用 `node:crypto`，`tsconfig.app.json` 的 `src` include 只允許瀏覽器安全程式碼，放 `src/lib` 會讓 `tsc -b` production build 失敗——已實測驗證此限制，未來若要新增類似資料準備模組請沿用 `scripts/` 慣例）、`tests/fixtures/clecTwReferenceV1.json`（versioned fixture，含完整 provenance）、`tests/clecTwReferenceHistoricalValidation.test.ts`（15 tests）。`package.json` 新增 `test:ur-todo-014-a` 並接入 `test:ci`。**未修改 `src/lib/clecHistoricalBacktest.ts`（既有 Foundation 原封不動重用）。**
- Reference Validation Portfolio v1（明文 validation-only，非 CLEC canonical／非 Production 預設／非 AppState role mapping／非使用者持股／非 UR-TODO-058）：prototype=0050、leveraged=00631L、cash-like=00865B。**UR-TODO-014-A2 延續同一組 mapping，不得變更此邊界。**
- 官方資料來源與存取方式：TWSE `STOCK_DAY_AVG`（股價，`https://www.twse.com.tw/exchangeReport/STOCK_DAY_AVG?response=json&stockNo={symbol}&date={YYYYMM01}`）與 TWSE `ETF 分配收益`（配息，`https://www.twse.com.tw/rwd/zh/ETF/etfDiv?stkNo={symbol}&startDate={YYYYMMDD}&endDate={YYYYMMDD}&response=json`）。兩者皆透過 Browser pane 唯讀 `navigate`＋`get_page_text` 逐字取得原始 JSON（非 AI 摘要、非第三方 adjusted close）；本機 shell `curl` 在此環境無法連外網，Browser pane 工具可連通，此為當時解除 STOP 狀態的關鍵技術路徑，**UR-TODO-014-A2 應延續同一取得方式**。
- Validation window（現況，PR #399 已 merge 內容）：2025-06-02～2025-08-29，daily，Asia/Taipei，60 個共同有效交易日、59 個 period。0050 官方 4:1 分割（last trading day 2025-06-10 收盤 188.65、suspension 2025-06-11～06-17、resumed 2025-06-18 收盤 47.57）已 split-adjust 正規化，2025-06-18 period return 僅 +0.86%（非人工製造的巨大單日跌幅）。0050 官方除息（2025-07-21，每受益權單位 0.36 元）採 distribution-inclusive return `(P_t+D_t)/P_previous-1`。00631L 官方 22:1 分割（2026-03-24 收盤 443.15、22:1 換算官方參考價 20.14、2026-03-31 回復交易）發生於本 dataset window 之外，未納入主 backtest periods，改以官方數值對做獨立 split-normalization regression evidence——**UR-TODO-014-A2 的最小建議範圍就是把這個分割正式納入主 window（延伸至涵蓋 2026-04-30 附近）**。00631L／00865B 全歷史（2010-01-01～2026-08-20 查詢範圍）官方配息紀錄皆為 0 筆；00865B 並與其正式基金契約「本基金收益不分配」條款交叉核對一致，無衝突，00631L 亦無衝突。
- 四組 CLEC preset 實際驗證結果（initialCapital=1,000,000、threshold={drift:5, significantMultiplier:2}）：442 totalReturnPct=25.13%／maxDrawdownPct=3.84%／rebalanceCount=0；433 totalReturnPct=21.30%／maxDrawdownPct=3.19%／rebalanceCount=0；703 totalReturnPct=29.68%／maxDrawdownPct=4.57%／rebalanceCount=0；5050 totalReturnPct=22.01%／maxDrawdownPct=3.32%／rebalanceCount=0。**四組 preset 於此 3 個月 window 內皆未觸發 full_rebalance，這是真實計算結果，不是人為篩選或期待值。既有 Foundation 測試（synthetic fixture）已完整覆蓋 full_rebalance path 本身的機制正確性；real-history full_rebalance path 尚未被任何真實資料覆蓋，這正是 UR-TODO-014-A2 要補的缺口。**
- **為何原 014-A dataset window 只有約 3 個月**（治理記錄補記，先前遺漏）：前一輪資料蒐集採 Browser pane 逐月手動取得 TWSE 官方資料，為控制單一 Sprint 的資料蒐集成本，選擇了「剛好涵蓋 0050 分割＋除息事件、把 00631L 分割留作獨立佐證」的最小範圍。**此為歷史實作理由，不是產品要求、不是 TWSE API 技術限制**——TWSE `STOCK_DAY_AVG`／`ETF 分配收益` 對任意月份／區間皆可正常查詢，已實測確認無技術阻礙。
- Fixture SHA-256：`d9c049f2f5b4045244bfa0842eea41e878839d1dfad0a116f20bd0202f36f338`（`computeDatasetHash()` 對 `tests/fixtures/clecTwReferenceV1.json` 原始檔案位元組計算，非對物件做 canonical JSON stringify）。若 UR-TODO-014-A2 延伸 window，此 hash 必然改變，須重新計算並更新所有引用處。
- 驗證結果（PR #399 merge 前後皆重新確認一致）：`npm run test:ur-todo-014`（既有 11/11，未改動）、`npm run test:ur-todo-014-a`（新增 15/15）、`npm run test:ci`（全數 pass）、`npm run build`（`tsc -b` 與 `vite build` 皆成功）。過程中 build 產生的本機 `dist/` 皆已用 `git checkout -- dist/` 還原，未污染 commit。
- 明確不包含（PR #399 Scope Guard，已 merge 且未觸碰；UR-TODO-014-A2 延續同一邊界）：UI、route、AppState、persistence、localStorage、JSON Backup schema、Financial Event Ledger、attribution、Production CLEC symbol mapping、Production Rebalance、Household Liquidity、historical Worker endpoint、runtime historical provider、UR-TODO-058、transaction-cost／slippage／next-day execution model。
- 下一位 AI／使用者：**UR-TODO-014-A2 尚未開始開發，不得未經使用者明確授權自行建立 branch 或開始實作。** 若使用者要求開始，範圍限於：延伸 `tests/fixtures/clecTwReferenceV1.json` window（重新執行 `scripts/clecTwReferenceDataset.ts` pipeline 產生新 fixture 版本並重新鎖定測試期望值，不得手動編輯 fixture 內的計算結果）、補齊延伸區間的官方 corporate-action provenance、讓 00631L 22:1 分割正式進主 backtest、重跑四組 preset 並如實記錄結果（無論是否出現真實觸發）。UR-TODO-014 整體收斂為 CLOSED 前，仍需另一輪正式 Closeout Audit。

## 最新交接快照：UR-TODO-071 Governance Closeout（CLOSED／Production Verified，2026-08-19）

- 正式決策：**UR-TODO-071（Holding Card Drag Reorder）= CLOSED／Production Verified。** PR [#397](https://github.com/hyc640110/family-universal-rebalance/pull/397) final head `78f44f5b0a55e50ff4c9d9fb845831dde3940649` 已由 `hyc640110` 於 2026-08-19T14:27:33Z 以一般 2-parent merge commit `e39f8489e95bc90cf37e46e060f8250ef04d0573` 合併（parents：`5d45ccd55a1bd3ef357edefe5d7369f0f29a4e0b`／`78f44f5b0a55e50ff4c9d9fb845831dde3940649`；未使用 admin override）。PR required CI `32262092171` success，相同 head Preview workflow_dispatch `32262111201` success，使用者完成 iPhone Safari Round 2 人工驗收 PASS；merge 後 main Deploy GitHub Pages `32264138787` success，head 與 merge commit 一致。
- 已完成契約：單一 ☰（`lucide-react` `Menu` icon，3 條水平線）drag handle 取代 UR-TODO-070 的 ↑/↓ 按鈕；`holdingDisplayOrder` 繼續作為唯一 display-order persistence，不重排 `AppState.holdings`、不 mutate `m.rows`，不影響 Rebalance／AI Decision／Financial Event Ledger／attribution／Firebase／財務公式。Native Pointer Events（Immediate Drag，無 long-press threshold，第一版不做 auto-scroll，未新增 drag-and-drop dependency）；Desktop（≥901px）handle 為整列最右側獨立操作區，詳細按鈕在其左側；Mobile（≤768px）handle 與 identity 共用 header 第一列。Keyboard 沿用既有單步 `moveHoldingDisplayOrder()`，新增 `aria-live="polite"` 排序結果播報。
- **開發歷程（Round 1 → Round 2 iPhone Safari 修正，重要實證，供未來類似 pointer/drag 功能參考）**：Round 1（element-level `setPointerCapture`＋`GripVertical` 六點 icon）於 iPhone Safari Preview 人工驗收發現 2 項 blocking issue：(1) 由上往下長距離拖曳失敗（由下往上正常）；(2) icon 不符核准的 ☰ 視覺要求，判定 FAIL／Refinement Required。**Root cause**：被 capture 的 handle 在每次 live-reorder 皆會被 React 重新定位到新的 array index（DOM 位置實際改變），Pointer Events spec 對「capture 元素被搬移（非移除）時是否保留 capture」屬 implementation-defined，Safari 相較 Chromium 更容易在元素重新定位時自動釋放 capture，導致長距離（多次重新定位）拖曳中途失效、短距離則較容易躲過此問題，與回報的方向性不對稱現象完全吻合。已用真實 React + jsdom harness 驗證純函式層本身邏輯完全正確，排除 index-space 計算錯誤的可能性。**Round 2 修正**：改用 `document` 層級的 `pointermove`／`pointerup`／`pointercancel` listener（`pointerdown` 時掛上、結束／取消／unmount 時卸除），完全不依賴 element pointer capture；icon 改為 `Menu`。Round 2 後 `npm run test:ur-todo-071`（41 tests）／`test:ur-todo-070`（25 tests）／完整 `test:ci`／typecheck／build 皆 pass，iPhone Safari 真機重新驗收 PASS。
- Production 唯讀驗證：HTTP 200；bundle 更新為 `index-DTD1MPZn.css`／`index-YNeFg2N5.js`；console 無錯誤；`#/assets` 頁面 4 張 holding cards 皆為單一 ☰ handle（原生 `<button>`，`role` 為 `null`，非 menu 語意）；`.holding-order-button` 舊控制項數為 0；Desktop 1280px 實測 ☰ 位於整列最右側、詳細按鈕在其左側；390px 無 horizontal overflow；1024px 維持 UR-TODO-070 既有、非本次引入的既存 overflow（未惡化）。全程僅用唯讀 DOM 查詢，未拖曳／未用 keyboard 排序 Production 使用者實際持股、未寫入 Production localStorage。
- **已知低風險 follow-up observation（非 blocking，供後續參考）**：`HoldingOrderHandle` 的 `handlePointerDown` 未防護「同一 handle 在前一次 drag 尚未 end/cancel 前又收到第二次 pointerdown」的情境（例如同一 handle 多點觸控），理論上可能造成該次 document listener 未被移除（記憶體洩漏，不影響排序資料正確性，因洩漏的 listener 只回應其已失效的舊 `pointerId`）。目前無 Production／iPhone 實際失敗證據，Round 2 真機驗收與 Production 唯讀驗證皆未觸發此情境；**目前無需另立新的 UR-TODO 編號**，後續若出現可重現問題再另行盤點。
- 本輪治理同步邊界：**只更新 `003_CURRENT_STATUS.md`／`008_TODO_BACKLOG.md`／`009_CHANGELOG.md`／`012_AI_HANDOVER.md` 與重新產生 Bundle，未修改 `src/**`／`tests/**`／`package*.json`／`.github/**`。**
...
- 下一位 AI：UR-TODO-070／UR-TODO-071 均已 CLOSED／Production Verified，兩者皆無剩餘待辦。若使用者提出與 duplicate-listener observation 相關的新需求，先確認是否有可重現的實際失敗證據，再決定是否需要建立新的獨立 UR-TODO；不得未經使用者要求就自行擴大範圍或搶先修正。

## 最新交接快照：UR-TODO-070 Governance Closeout（CLOSED／Production Verified，2026-08-19）／UR-TODO-071 新增 OPEN／PLANNED（歷史快照，已由本文件最上方 UR-TODO-071 CLOSED 條目取代）
...
- 邊界：不改 matching priority、one-to-one algorithm、externalId identity、fingerprint、possible identity、`posted && !excluded` filter、schema、persistence、AppState、JSON Backup、ImportSession、duplicate formula、UR-TODO-022-A／B／C 或 transaction write path。`test:ci` 1,407 pass，TypeScript、Production／Preview build、`git diff --check` pass；npm audit 的 4 個 high vulnerabilities 為既有相依問題，未由本 Sprint 引入。
