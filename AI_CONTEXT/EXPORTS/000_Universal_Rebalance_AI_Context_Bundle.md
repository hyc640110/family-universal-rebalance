...
Generated UTC: 2026-08-20T14:22:37.387300+00:00
...
- `003_CURRENT_STATUS.md` — SHA-256 `5de8bdbdd6dd291afd37f4abf2b7cc7c8379e1a68c621dd36fbab468d70994e2`
...
- `008_TODO_BACKLOG.md` — SHA-256 `217d60f0ba20adfae976fb7aeab101524707d598353f61204b8bc30e8d4f3939`
...
- `012_AI_HANDOVER.md` — SHA-256 `e4d5ecca312595dd89724b1ea821ed1f5802f2fcaa30ab5333b33c81acfde3d9`
...
**UR-TODO-014-A2（Extended Real-History Trigger Validation）開發完成，PR [#401](https://github.com/hyc640110/family-universal-rebalance/pull/401) 已開為 Draft，尚未 Merge。** Branch `feat/ur-todo-014-a2-real-history-trigger` 自 `origin/main` `2e1399ffaa6b4c41564e5a0ac46dc49a289358c7` 開出，final head `ab78fb2`。延伸既有 UR-TODO-014-A validation-only reference fixture（prototype=0050、leveraged=00631L、cash-like=00865B，仍非 CLEC canonical mapping／Production 預設／AppState role mapping／使用者持股／UR-TODO-058）window 從 `2025-06-02～2025-08-29` 延伸至 `2025-06-02～2026-04-30`，同一 TWSE 官方 `STOCK_DAY_AVG`／`ETF 分配收益` 唯讀 pipeline，將 00631L 22:1 分割（原僅 standalone evidence）正式納入主 backtest window。**AC1 已滿足：找到真實歷史資料產生的 `full_rebalance` 觸發**——`clec-5050` 於 `2025-09-12`（target leveraged=50，drift 10.354pp ≥ threshold 5×2=10pp；前一日 2025-09-11 drift 9.818pp 仍為 `rebalance_consider`）；四組 preset 現皆至少觸發一次（442／433 於 2026-01-05；703 於 2025-10-07 與 2026-04-22；5050 於 2025-09-12 與 2026-01-27；原本皆為 0）。共同有效交易日 214（原 60）、period 213（原 59），fixture SHA-256 更新為 `a8d40a12c2ce3e840ee3671d014ddc040e0f28226ecb03f705fddb37b9c936ad`（原 `d9c049f2f5b4045244bfa0842eea41e878839d1dfad0a116f20bd0202f36f338`）。**開發過程中發現並經使用者授權修正 `src/lib/taiwanTradingCalendar.ts` 既有 2025 年曆缺口**：`2025-09-29`（教師節適逢週日之補假）與 `2025-10-24`（臺灣光復節適逢週六之補假）為官方國定假日補假，原清單缺漏，經使用者確認官方依據後授權新增，並補上 regression test；此為本 Sprint 額外必要修改，範圍嚴格限於這兩個已有正式依據的日期，不含任何「多檔同時缺資料即視為休市」的推論邏輯。`src/lib/clecHistoricalBacktest.ts`／`clecStrategyRules.ts`／`allocationPresets.ts` 三個 CLEC 核心檔案完全未修改，沿用既有 Production-verified Foundation。`npm run test:ur-todo-014`（11/11）、`test:ur-todo-014-a`（17/18，唯一未過項目為本機 Windows `core.autocrlf` CRLF checkout 造成的既有環境問題，非本次改動所致，已驗證 git 內容為 LF、CI 應可通過）、`test:ci`（分段驗證全數 pass）、`tsc -b`、`build`、`git diff --check` 皆通過。**尚待 Preview／使用者驗收與 Merge 授權，AI 未自行 Merge。**
...
- Branch／PR facts：Branch `feat/ur-todo-014-a2-real-history-trigger` 自 `origin/main` `2e1399ffaa6b4c41564e5a0ac46dc49a289358c7`（PR #400 governance closeout merge）開出，final head `ab78fb2`。PR #401 為 Draft，未使用 admin override，AI 未自行 Merge。
...
- 已完成契約：延伸既有 validation-only Reference Portfolio（prototype=0050、leveraged=00631L、cash-like=00865B，邊界與 014-A 完全相同，**不得**升格為 CLEC canonical mapping／Production 預設／AppState role mapping／使用者持股／UR-TODO-058）的真實歷史 window，從 `2025-06-02～2025-08-29` 延伸至 `2025-06-02～2026-04-30`，沿用同一 TWSE 官方 `STOCK_DAY_AVG`（`https://www.twse.com.tw/exchangeReport/STOCK_DAY_AVG?...`）／`ETF 分配收益` 唯讀 Browser pane pipeline，**未新增資料來源**。共同有效交易日 60→214、period 59→213；fixture `datasetVersion` 1.0.0→2.0.0，SHA-256 `d9c049f2f5b4045244bfa0842eea41e878839d1dfad0a116f20bd0202f36f338` → `a8d40a12c2ce3e840ee3671d014ddc040e0f28226ecb03f705fddb37b9c936ad`。
...
- 驗證結果：`npm run test:ur-todo-014`（既有 11/11，Foundation 未改動）、`npm run test:ur-todo-014-a`（本機 17/18；唯一未過項目為 test `K`，對 `scripts/clecTwReferenceDataset.ts` 做字面 `\n`-only regex 比對，本機 Windows `core.autocrlf=true` checkout 會將該檔案轉為 CRLF 導致比對失敗——**已用 `git stash` 驗證此問題在 A2 任何改動前、對完全未修改的原始檔案就已存在**，並用 `git show HEAD:` 取出 git 實際儲存的 LF 內容驗證能通過同一正則，確認這是純本機環境假性失敗、非本次改動造成的 regression，CI（Linux runner）預期可正常通過）、`test:ci` 全鏈因上述本機假性失敗會中斷 `&&` chain，已改為逐一手動執行鏈中每一個 npm script（含 `test:ur-todo-022-a`、firebase retirement regression、`test:risk-presentation`、`test:ci:unit-mjs`、`test:ci:checks`、全部 attribution／loan／fx／runtime-attribution 系列、`test:ur-todo-016`／`070`／`071`、`pretest:ci`／`posttest:ci`）確認全數 pass、共 1,089+ 測試無 fail。`npx tsc -b`：本機環境原本缺少 `pdfjs-dist`（`package.json` 已宣告但 `node_modules` 未安裝，既有環境缺口非本次改動造成），已執行 `npm install` 補齊後 `tsc -b` 乾淨通過。`npm run build`：pass。`git diff --check`：pass，無 whitespace 錯誤。
