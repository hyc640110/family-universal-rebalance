...
- `009_CHANGELOG.md` — SHA-256 `2fe95c159008e112277141fe147c731cd624d4bbe208472a2b7157d4e41ab8b5`
...
- 優先級：待評估
- 狀態：**部分完成／後續待評估**（Phase 1 唯讀盤點、C1 Financial Event Ledger contract／persistence foundation、046-B pure attribution calculator／quality model、046-C1／C2 pure transaction reconciliation、046-C3A pure runtime derived-evidence adapter、046-C3B runtime attribution composition layer、046-C3C-A runtime attribution provenance card、046-C3C-B session-only mark-as-reasonable toggle、046-C3C-C Financial Event Ledger 寫入／持久化、Firebase Ledger Sync、撤銷／void、046-I1 investment trade contract，以及 046-L1 loan repayment contract 已完成；UR-TODO-043-B 依賴已解除；其餘子階段（split allocation、FX attribution、Loan UI／CSV／Import Center consumer mapping 等重大事件）尚未排程或開發）
- 提出日期：2026-07-30
