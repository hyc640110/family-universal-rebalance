...
Generated UTC: 2026-08-13T14:09:30.484689+00:00
...
- `003_CURRENT_STATUS.md` — SHA-256 `cf48cb0c68129f0f5220532ba58e20f7e63bb68512192dc623f27467c3d4a684`
...
- `008_TODO_BACKLOG.md` — SHA-256 `44ea94e447db06201340a674ca060a57129f1b08eb4a0f5e525680576d3e0115`
- `009_CHANGELOG.md` — SHA-256 `8870424828a1e5e5c6d58bd31cf17545c8a8d194c4469c358e663ded7fd6ac1c`
...
- `012_AI_HANDOVER.md` — SHA-256 `4cf6b3043f1e6b8cc1d76619923d6beea6a3ee4f5352eab35200fe2cf171f47e`
...
- `020_Architecture_Decisions.md` — SHA-256 `813e517aa399ceef52507f72b9400e7680fa8a4bbd803531821c459f29fddc12`
...
- **Remaining Boundaries（FX-A1 Merge 後更新）**：
  ...
  - **FX-F1D Controlled Producer Feature Gate Foundation**：開發完成，Draft PR 待架構審查（尚未 Merge）。落地 Code Constant Narrow Gate（`src/lib/fxOpaqueProducerGate.ts`），Production 恆為 OFF，Preview 目前亦為 OFF（source gate 未解鎖）；風險降低工具，非 absolute compatibility guarantee，未解決 F1C 已確認的 legacy-client 限制。
...
- **下一正式候選（待盤點，未開始）**：FX-A3 已完成；FX-F1A 已完成／Merge；FX-F1B／FX-F1C 唯讀盤點已完成；FX-F1D Draft PR 待架構審查與 Merge。FX-F1D Merge 後，下一候選必須先由使用者針對「第一個 opaque FX producer」明確授權（含 adoption window、manual upgrade SOP、Backup 前置要求，見 FX-F1D 架構審查報告），不得因 gate 存在而自動視為已解鎖；其餘候選（FX attribution evidence/runtime integration、conversion／realized FX、foreign investment／loan，或 Loan UI／CSV／Import Center producer mapping）仍待獨立評估。**若下一候選涉及 Ledger 寫入語意、schema／persistence 結構性變更、核心 attribution 結果或 AI Decision／Rebalance／Household Liquidity 接線，屬重大產品／核心財務語意事件，須另行拍板，不得自動開始。** UR-TODO-046 整體維持「部分完成／OPEN」，不得因 FX-A1、FX-A2、FX-A3 或 F1A～F1D 完成自行標記整體完成。
...
## 最新交接快照：UR-TODO-046 FX-F1D Controlled Producer Feature Gate Foundation（開發完成／Draft PR 待架構審查，2026-08-13）
...
- 下一直接起點：Draft PR 待 CI／diff／架構審查；**F1D 是 controlled-rollout risk reduction 工具，不代表已解決 legacy client retroactive protection 問題；第一個 opaque producer 仍需另行明確授權（含 adoption window、manual upgrade SOP、Backup 前置要求），不得因本 Sprint 完成或 source gate 未來被翻成 `true` 而自動視為已解鎖**。UR-TODO-046 整體仍 OPEN；**不得自行 Merge、Ready for review 或部署 Production**；PR #322（Loan audit）為獨立 Draft／OPEN，本次未處理。
