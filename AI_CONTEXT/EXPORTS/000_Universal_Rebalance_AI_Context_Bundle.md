...
- **下一正式候選（待盤點，未開始）**：FX-A3 已完成；下一候選依序獨立評估 FX attribution evidence/runtime integration、conversion／realized FX、foreign investment／loan，或 Loan UI／CSV／Import Center producer mapping。**若下一候選涉及 Ledger 寫入語意、schema／persistence 結構性變更、核心 attribution 結果或 AI Decision／Rebalance／Household Liquidity 接線，屬重大產品／核心財務語意事件，須另行拍板，不得自動開始。** UR-TODO-046 整體維持「部分完成／OPEN」，不得因 FX-A1、FX-A2 或 FX-A3 完成自行標記整體完成。
