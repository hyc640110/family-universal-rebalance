...
Generated UTC: 2026-08-14T06:02:55.535507+00:00
...
- `003_CURRENT_STATUS.md` — SHA-256 `7422b549326df92d3159036e7f14cffa04ed3a9c8cb5598fee38f40de3257d13`
...
- `008_TODO_BACKLOG.md` — SHA-256 `23d07248ad2027efa782e9ccb681221f2831cfc4cb889415c8987b1b24568f7d`
- `009_CHANGELOG.md` — SHA-256 `3ae169602ad5c100e8640746b15d9dcf504b080f271b32524143f8c85b62b706`
...
- `012_AI_HANDOVER.md` — SHA-256 `1cb80e284fdf1cb25f664b6096fdae1cfb538af4e1bd189eb9a36f8e4a7cda1a`
...
# Universal Rebalance Current Status v4.02
...
# Universal Rebalance Todo Backlog v1.79
...
- 優先級：待評估
- 狀態：**部分完成／後續待評估**（Phase 1 唯讀盤點、C1 Financial Event Ledger contract／persistence foundation、046-B pure attribution calculator／quality model、046-C1／C2 pure transaction reconciliation、046-C3A pure runtime derived-evidence adapter、046-C3B runtime attribution composition layer、046-C3C-A runtime attribution provenance card、046-C3C-B session-only mark-as-reasonable toggle、046-C3C-C Financial Event Ledger 寫入／持久化、Firebase Ledger Sync、撤銷／void、046-I1 investment trade contract、046-L1 loan repayment contract、L2C-P0 status contract、L2C-P1 forensic no-recovery conclusion 與 L2C-P2 missing-ledger compatibility guard、FX-A1／FX-A2／FX-A3、FX-F1A Transaction Opaque Compatibility Foundation、FX-F1B Consumer Guard Audit、FX-F1C Producer Rollout Contract Review、FX-F1D Controlled Producer Feature Gate Foundation、FX-F2A First Producer Contract Audit、FX-F2B Pairing Identity Contract Foundation、FX-F2C Manual FX Conversion Producer Contract Review、FX-F2C-1 Minimal Consumer Guard 已完成／Merge；FX-F2C-2 Manual FX Conversion Producer 開發完成待 Merge（gate 仍 OFF，Production／Preview 皆無法真正建立 FX conversion）；UR-TODO-043-B 依賴已解除；其餘子階段（split allocation、FX attribution evidence/runtime integration、conversion／realized FX、foreign investment／loan、Preview capability enable、Loan UI／CSV／Import Center consumer mapping 等重大事件）尚未排程或開發，Preview capability enable 另需獨立明確授權與獨立 PR，不因 F2C-2 Producer 程式碼已存在而自動解鎖）
- 提出日期：2026-07-30
...
## 最新交接快照：UR-TODO-046 FX-F2D Attribution Integration（開發完成／Draft PR 待 CI／Preview 驗收，2026-08-14）
...
- 下一直接起點：Draft PR 待建立（`feat/ur-todo-046-fx-f2d-attribution`）；PR CI 通過後需觸發真正的 `workflow_dispatch` Preview 部署以進行正式 E2E 驗證（建立 FX → candidate → confirm → matched → contribution 0 → void → candidate → reconfirm → matched；confirmed 狀態下嘗試刪除原始交易應被攔截），本機 build 模擬驗證不能取代正式 Preview 環境驗收。**本 Sprint 只完成 attribution 接線本身，不代表 Production Producer 已解鎖，也不代表 UR-TODO-046 已全數完成**；完成後停止，交回 ChatGPT 做 F2D 最終架構與 Merge Gate 審查。
