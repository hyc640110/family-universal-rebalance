...
- `009_CHANGELOG.md` — SHA-256 `e15cd060fb0cea69439fef22ad804b910b3a8afca25d2a011d6fd76a8a6fbf0c`
...
**UR-TODO-046-I1 Investment Trade Contract & Fail-safe Reconciliation Foundation（Draft PR [#292](https://github.com/hyc640110/family-universal-rebalance/pull/292)，2026-08-09）**：新增向後相容 `FinancialTransaction.investmentAttribution` discriminated union，並完成 `income-other` regression、fee／tax settlement 防重複與 trade＋cash movement fail-safe 收斂。完整正式 TWD buy／sell 優先於 generic taxonomy，買／賣本金 contribution 固定為 0；一般、未附完整 trade contract 的 `income-other` 維持既有 `external-income`。fee／tax 僅在明確 `costId`、`settlementCostTreatment: independent` 且可唯一連結正式 trade 時產生一次負 contribution；`included`／`unknown`／legacy／重複／無關聯一律不扣除。另建 cash movement 必須有 explicit `cashMovementId`、`kind: cash-movement`、方向、相同帳戶與幣別的唯一對應，否則不猜測、不歸因；Ledger confirmation 同樣套用防護。legacy、文字、類別、金額正負號、缺欄位、non-TWD、duplicate trade identity、realized gain/loss 一律 fail-safe。採 additive fields，無 schema bump／migration。程式修正 commit `b37ddaf476af87c63d566718087605c27a6a953d` 的 CI `31299265540` success；Draft 尚未 Merge 或部署。
