...
Generated UTC: 2026-08-12T11:14:56.240349+00:00
...
- `003_CURRENT_STATUS.md` — SHA-256 `af15d83cd49b383baea16bf55cb440c74903bd10a74e592d13c3d703fb5c2308`
...
- `008_TODO_BACKLOG.md` — SHA-256 `0aac9ff815f91a4e06aad413a9a80b9638a717ff9da6ff5ad3107f84d51a118c`
...
- `012_AI_HANDOVER.md` — SHA-256 `dc344fbc1aa39a4ac8a636c2efebfae5a9421f41bc9ec62f6575efde7b9a753d`
...
**UR-TODO-001 Firebase Retirement：P3-B2-A、P3-B2-B、P3-B2-C 與 P3-B3-A 已正式完成；P3-B3-B Canonical AppState Firebase Retirement 為現行 Draft 候選。** 正式 `origin/main` 為 PR [#312](https://github.com/hyc640110/family-universal-rebalance/pull/312) merge commit `1bbff5b8dd06223be1a00c2c1b92457396355076`（`mergedAt: 2026-08-12T10:14:16Z`；`mergedBy: hyc640110`；未使用 admin override）。現行 App 的 Firebase Auth、RTDB GET／PUT、token refresh、upload/download UI、remote Ledger merge 與 Firebase SDK runtime 均為 0。localStorage 是唯一 canonical device persistence；JSON Backup 是正式人工備份／裝置搬移；Financial Event Ledger 維持本機 persistence contract。P3-B3-A 保留 legacy localStorage／Backup tolerant-read，新的 canonical localStorage／JSON Backup 不再輸出 `autoSync`、`autoSyncSec`、persisted `workerUrl`、Firebase baseline metadata、`lastUploadAt` 或 `lastDownloadAt`；無 schema bump、Backup version bump、migration 或歷史 localStorage 主動 rewrite。P3-B3-B 移除 canonical `AppState.firebase`／`FirebaseConfig`，legacy top-level／nested Firebase input 採 accept-and-discard，並保持 legacy-only hydration 不自動改寫。`syncMeta.dirty`／`source`／local-Backup timestamps、`FIREBASE_BASE_PATH`／`VITE_FIREBASE_BASE_PATH` 與 `mergeFinancialEventLedgers()` 均保持不變。P3-B3-C environment naming cleanup 與 P4 Console 尚未開始，禁止操作；**UR-TODO-001 仍 OPEN。**
...
2026-08-12 **UR-TODO-001 Firebase Retirement P3-B2-A／P3-B2-B／P3-B2-C 與 P3-B3-A 已正式完成；P3-B3-B 為現行 Sprint**。P3-B2-A／PR [#309](https://github.com/hyc640110/family-universal-rebalance/pull/309) merge commit `15a8f3466dd8ba09b3d68de935749b3ff9257f4c`，退休 Firebase sync dead runtime 並建立 runtime absence regression guard；P3-B2-B／PR [#310](https://github.com/hyc640110/family-universal-rebalance/pull/310) 一般 Merge，`origin/main`／merge commit `4262c565aee309074b16ad083947e6dd52a96664`（`mergedAt: 2026-08-11T16:16:20Z`；`mergedBy: hyc640110`；未使用 admin override），新 JSON Backup 不再輸出 `syncSettings.firebase`／`firebaseConfigured`，但舊 top-level／nested Firebase Backup 仍可讀。P3-B3-A／PR [#312](https://github.com/hyc640110/family-universal-rebalance/pull/312) 一般 Merge，`origin/main`／merge commit `1bbff5b8dd06223be1a00c2c1b92457396355076`（`mergedAt: 2026-08-12T10:14:16Z`；`mergedBy: hyc640110`；未使用 admin override），退休新的 canonical cloud-sync output 並保護 legacy-only hydration。現行 Firebase Auth、RTDB GET／PUT、token refresh、upload/download UI、remote Ledger merge 與 Firebase SDK runtime 均為 0；localStorage 是 canonical device persistence，JSON Backup 是人工備份／裝置搬移，Financial Event Ledger contract 保留。P3-B3-B 只退休 canonical `AppState.firebase`／`FirebaseConfig`，舊 Firebase input 必須 accept-and-discard；P3-B3-C environment naming cleanup、P4 Console／RTDB／Auth／Rules／Project／remote data／legacy browser Auth session 尚未開始且禁止操作。`mergeFinancialEventLedgers()` 是通用 Ledger contract，**KEEP**，不列為 Firebase retirement blocker。
...
#### 後續延伸：Firebase Retirement Phase（方案 B 已批准；P0、P1、P2-A、P3-A1、P3-B1、P3-B2-A、P3-B2-B、P3-B2-C、P3-B3-A 已完成；P3-B3-B 為 Draft 候選）
