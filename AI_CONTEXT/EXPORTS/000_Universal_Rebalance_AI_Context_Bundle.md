...
- `009_CHANGELOG.md` — SHA-256 `a76082adf4789c0939d9946552a5ef26f9e5a0b645bcc782e622a4447342e5dd`
...
**UR-TODO-014-A CLEC Taiwan Reference Historical Validation（2026-08-20，implemented／verified，pending Closeout Audit；UR-TODO-014 整體仍 OPEN，尚未 Merge）**：新增 `scripts/clecTwReferenceDataset.ts`（純資料準備 pipeline，重用既有未修改的 `runClecHistoricalBacktest()` Foundation）＋ versioned offline fixture `tests/fixtures/clecTwReferenceV1.json`（`clec-tw-reference-v1`）＋ `tests/clecTwReferenceHistoricalValidation.test.ts`（15 tests，`npm run test:ur-todo-014-a`，已納入 `test:ci`）。Reference Validation Portfolio v1（validation-only：prototype=0050、leveraged=00631L、cash-like=00865B）以 TWSE 官方 `STOCK_DAY_AVG`（股價）與 `ETF 分配收益`（配息）為唯一資料來源，2025-06-02～2025-08-29（60 共同有效交易日）完整正規化 0050 官方 4:1 分割與除息（distribution-inclusive return），並以獨立官方數值對驗證 00631L 官方 22:1 分割證據（發生於 window 之外）。442／433／703／5050 四組 preset 對此 window 的真實驗證結果：totalReturnPct 分別為 25.13%／21.30%／29.68%／22.01%，maxDrawdownPct 分別為 3.84%／3.19%／4.57%／3.32%，rebalanceCount 皆為 0（真實計算結果，非人為篩選）。`test:ur-todo-014`（既有 11/11）、`test:ur-todo-014-a`（新增 15/15）、`test:ci`、`build` 全數通過。UI／transaction-cost model 已拍板移出 UR-TODO-014 closure requirement（CONDITIONAL）。詳見 `008_TODO_BACKLOG.md`／`012_AI_HANDOVER.md` UR-TODO-014 正式條目。
