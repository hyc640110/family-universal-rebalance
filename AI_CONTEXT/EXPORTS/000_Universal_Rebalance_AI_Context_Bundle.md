...
Generated UTC: 2026-08-11T14:52:19.679436+00:00
...
- `003_CURRENT_STATUS.md` — SHA-256 `f88152528e8d8647995644940f7c916e91f11341bf2eab36a84513e95f1f0dd7`
...
- `008_TODO_BACKLOG.md` — SHA-256 `ca7a767e8d02f850d346b610abdf49d573d6b0da807fd34f71179f7c423c7aa8`
- `009_CHANGELOG.md` — SHA-256 `bf3b975fa7cc6760a2ca9105698f12b2a3a54e8525b771ee6b0ec6d9694c6ce3`
...
- `012_AI_HANDOVER.md` — SHA-256 `81976b5f0726295fb7b3c6eddbe1b5d0371157c3b76d72d07cec66d8ea2beb1f`
...
# Universal Rebalance Current Status v3.93

最後更新：2026-08-11

**UR-TODO-001 Firebase Retirement P3 Repository 唯讀盤點與 P3-A1（Firebase Runtime Remnants Cleanup）已正式完成；P3-B1 Legacy Contract Adapter 為 Draft，尚未部署或 Merge。** P2-A 已由 PR #304 Merge；P3-A1 由 PR [#305](https://github.com/hyc640110/family-universal-rebalance/pull/305) 正常 Merge，merge commit `78e50c3d09f122b18d968ebcddf0bd2b52bf177f`（parents：`339f8c305a419117af54f4dbd69a3b47b903a26c`、`40101739a1429aac5bd6ecf0a13d910ac397a5b6`；`mergedAt: 2026-08-11T14:20:24Z`；`mergedBy: hyc640110`；未使用 admin override）。P3-A1 移除零 production caller 的 Firebase Anonymous Auth runtime module、Firebase RTDB URL builder runtime module、直屬 tests、`VITE_FIREBASE_API_KEY` 與程式端 API key／Auth session key dead constants；production Firebase Auth／RTDB runtime reference 已為 0。P3-B1 採「Backward-readable, not backward-re-export」：舊 `firebase`、`syncSettings.firebase`、`firebaseConfigured` 可讀；僅真實持久化操作或明確 Full Restore 才產出不含 top-level `firebase` 的 canonical localStorage，首載／reload／瀏覽／read normalization 不得為 legacy-only delta 寫入。乾淨 Backup 不得 fallback 到 current Firebase config；Backup 匯出 cleanup、`syncState.ts`、Ledger、`VITE_FIREBASE_BASE_PATH` 與 Firebase Console 保持未處理。Production Pages workflow `31500994060` success，Production HTTP 200、metadata=`production`、asset=`index-D4cszGRQ.js`；Preview HTTP 200、metadata=`preview`、asset=`index-DCUgxpdq.js`，兩者路徑隔離正常。既有瀏覽器 `${STORAGE_KEY}:firebase-auth-session` 未清除。P3-B2 與 P4 Firebase Console、RTDB、Auth provider、Rules、Project retirement 均需另行明確授權。
...
2026-08-11 **UR-TODO-001 Firebase Retirement P3-B1 Legacy Contract Adapter（Draft，未 Merge／未部署）**：產品決策採 **Backward-readable, not backward-re-export**。舊 localStorage 與 Backup 的 top-level `firebase`、`syncSettings.firebase`、`firebaseConfigured` 保留 read compatibility，沒有 Firebase functional runtime／canonical 意義；不得在 startup、reload、瀏覽、read normalization、legacy-only delta 或 mount effect 自動清理／回寫。僅 genuine persistent user mutation 或明確 Backup Full Restore 後，canonical localStorage writer 才移除 top-level `firebase`；clean Backup 缺少 Firebase 欄位時不得 fallback 到 `current.firebase`。本階段不清理 Backup export 的 `syncSettings.firebase`／`firebaseConfigured`，不碰 `syncState.ts`、autoSync／syncMeta、Ledger／schema、`VITE_FIREBASE_BASE_PATH`、environment boundary、Firebase Console 或 stale browser auth session。P3-B2 尚未開始；任何 legacy 欄位的 read-time／localStorage／JSON Backup 進一步策略仍須另行決策。
...
**UR-TODO-001 Firebase Retirement P3-B1 Legacy Contract Adapter（2026-08-11，Draft）**：以 `origin/main` `130e673257585c8dcd8733e35cd6c0491bb544ef` 為基線，正式採用「Backward-readable, not backward-re-export」。新增 Firebase legacy persistence adapter 與 explicit initial hydration write gate：舊 top-level `firebase`、`syncSettings.firebase`、`firebaseConfigured` 可讀；當且僅當 genuine persistent mutation 或明確 Backup Full Restore 發生，canonical localStorage output 才移除 top-level `firebase`。首載／reload／瀏覽／read normalization／legacy-only delta 與 mount effect 均不得自動回寫；clean Backup 不再 fallback 到 `current.firebase`。未清理 Backup export、`syncState.ts`、autoSync／syncMeta、Ledger、schema、`VITE_FIREBASE_BASE_PATH`、environment boundary、Firebase Console 或 browser auth session；P3-B2 未開始。Draft 僅完成本機驗證，未 Preview、未 Merge、未部署。
