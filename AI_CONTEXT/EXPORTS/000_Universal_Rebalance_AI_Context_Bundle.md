...
Generated UTC: 2026-08-13T10:12:37.128371+00:00
...
- `003_CURRENT_STATUS.md` — SHA-256 `f2bfe3737c73644b088bca53385fa765ce321e9fe40dfd5791f4053a7abc21f8`
...
- `008_TODO_BACKLOG.md` — SHA-256 `cf269855cc32b3272516a0e455c2d440696a3a01b6bbbd5d9895f8fa6cf6c479`
- `009_CHANGELOG.md` — SHA-256 `80a89a0d9ed6268425229ab1db7ea2b2e6a2e72477999fb20cfc9e107853e52b`
...
- `012_AI_HANDOVER.md` — SHA-256 `9ecb858173958250f48a3aa21b43a321bba115600f87074f9fd6d5bfa69faaf6`
...
**UR-TODO-046 Loan principal／interest attribution Repository 唯讀盤點：NO-GO DEVELOPMENT，既有 contract 已完整覆蓋，Repository audit confirmed（2026-08-13）。** 依使用者要求先確認既有 `loan-payment` atomic contract（UR-TODO-046-L1，PR #294）是否已足以覆蓋 loan principal／interest attribution，而非預設需要開發或改走 Generic Split Consumer。以實際程式碼逐條實證：principal 恆為 0、interest／fee／penalty 恆為一次性負貢獻（`src/lib/loanAttribution.ts`、`src/lib/netWorthAttribution.ts`）；`componentId`＋`paymentId`＋`confirmationGroupId` 三層 stable identity 防止重複（`src/lib/loanAttributionContract.ts`）；`appendFinancialEventGroup()`／`resolveActiveLoanComponentGroups()`（`src/lib/financialEvents.ts`）提供原子寫入與完整性守恆，不與 Generic Split 共用通道；`transactionReconciliation.ts`／`runtimeAttributionComposition.ts` 的 candidate→matched 升級與三層 double-count 防護皆有程式碼與測試互證；void 一個 component 會使整組正確回退為 candidate、contribution 正確從 Ledger 路徑轉回 derived 路徑，不漏記、不重複；localStorage／JSON Backup round-trip 完整保留，schema v1/v2/v3 有明確 gate。**發現並修補一項測試覆蓋缺口（非程式碼邏輯缺口）**：新增 1 個 regression test（`tests/loanComponentGroup.test.ts`）驗證「兩個完整、皆 posted、皆未 void、共用同一 paymentId 的 confirmation group」會被 `resolveActiveLoanComponentGroups()` 正確 fail-safe 排除，測試通過且**零生產程式碼變更**。`npx tsc -b`、`npm run test:ci`、Production／Preview build 皆成功。**結論：不建立第二套 Loan attribution 架構，不改 `netWorthAttribution.ts` 核心公式，不把 Loan 搬到 Generic Split。** UR-TODO-046 下一真正缺口轉向 FX attribution evidence／runtime integration；UR-TODO-046 整體仍 OPEN。
...
2026-08-13 **UR-TODO-046 Loan principal／interest attribution Repository 唯讀盤點：NO-GO DEVELOPMENT，既有 contract 已完整覆蓋，Repository audit confirmed。** 依使用者要求，先確認既有 `loan-payment` atomic contract（UR-TODO-046-L1，PR #294）是否已完整覆蓋 loan principal／interest attribution，而非預設需要開發或改走 Generic Split。以實際程式碼逐條實證確認：(A) 經濟語意——`src/lib/loanAttribution.ts` 的 `deriveLoanRuntimeEvidence()` 與 `src/lib/netWorthAttribution.ts` 的 `ZERO_EFFECT_EVENT_TYPES`／負貢獻分類，principal 恆為 0、interest／fee／penalty 恆為一次性負貢獻，`derived` 與 `Ledger` 兩條路徑結果一致（`tests/loanAttribution.test.ts` 實測 principal=0、interest=-5,000）；(B) identity——`componentId`＋`paymentId`＋`confirmationGroupId` 三層 key，`src/lib/loanAttributionContract.ts` 的 `hasUniqueLoanComponentIdentity()` 防止同一 `loanId` 下 `componentId` 跨 payment 重複；(C) atomic contract——`appendFinancialEventGroup()`（`src/lib/financialEvents.ts:577-597`）原子寫入、`resolveActiveLoanComponentGroups()`（同檔 316-382）要求 group 內所有 component 皆 `posted` 且總額守恆 `settlementAmount`，pending／mixed／不完整一律整組不生效，不會與 Generic Split 共用通道（`genericSplitAllocation.ts:50` 明確排除 `domain==='loan-payment'`）；(D) pipeline——`transactionReconciliation.ts` 產生 `candidate`（`loan-payment-contract-candidate`），group 完整確認後由 `runtimeAttributionComposition.ts:174-182` 升級為 `matched`（`linked-loan-payment-group`），三層程式碼防止 double-count（已確認 payment 不再產生 derived evidence、derived transactionId 去重、Ledger 側 componentLink 未驗證通過即丟棄）；(E) void／replacement——void 任一 component 會使整組（含未被 void 的其他 component）判定不完整、reconciliation 回退為 `candidate`、contribution 由 Ledger 路徑正確轉回 derived 路徑（`tests/loanComponentGroup.test.ts` 端到端斷言 `ledgerContribution -5000→0`、`derivedContribution 0→-5000`）；replacement 為隱式（void 舊 component 使舊 group 整組失效後才能被新 group 取代，無顯式 `replacementOfGroupId` 欄位，與 Generic Split 的顯式機制不同，但不影響正確性）；(F) persistence——localStorage／JSON Backup round-trip 完整保留 `loanAttribution`／`componentLink`，`FINANCIAL_EVENT_SCHEMA_VERSION=3` 明確 gate v1（不支援 componentLink）／v2+（loan component 類型必須有 componentLink）（`tests/financialEventPersistence.test.ts` 實測）。**發現並修補一項測試覆蓋缺口（非程式碼邏輯缺口）**：`resolveActiveLoanComponentGroups()` 的 `paymentCounts.get(paymentId) !== 1` fail-safe 理論上會排除「兩個完整、皆 posted、皆未 void、共用同一 paymentId」的 duplicate confirmation group，但先前沒有測試直接斷言這個具體情境；已在 `tests/loanComponentGroup.test.ts` 新增 1 個 regression test 驗證該行為（雙方皆被排除、`ledgerContribution=0`、正確 fallback 到單一 `derivedContribution=-5,000`，不 double count、不漏記），**測試通過且零生產程式碼變更**，證實現行 contract 本已正確處理此案例。`npx tsc -b`、`npm run test:ci`、Production／Preview build 皆成功，`git diff --check` pass。**結論：不建立第二套 Loan attribution 架構，不改 `netWorthAttribution.ts` 核心公式，不把 Loan 搬到 Generic Split。** UR-TODO-046 下一真正缺口轉向 FX attribution evidence／runtime integration（或 Loan UI／CSV／Import Center producer mapping，屬 delivery boundary非 attribution consumer gap）。UR-TODO-046 整體仍 OPEN。
...
- 優先級：待評估
- 狀態：**部分完成／後續待評估**（Phase 1 唯讀盤點、C1 Financial Event Ledger contract／persistence foundation、046-B pure attribution calculator／quality model、046-C1／C2 pure transaction reconciliation、046-C3A pure runtime derived-evidence adapter、046-C3B runtime attribution composition layer、046-C3C-A runtime attribution provenance card、046-C3C-B session-only mark-as-reasonable toggle、046-C3C-C Financial Event Ledger 寫入／持久化、Firebase Ledger Sync、撤銷／void、046-I1 investment trade contract、046-L1 loan repayment contract、L2C-P0 status contract、L2C-P1 forensic no-recovery conclusion 與 L2C-P2 missing-ledger compatibility guard 已完成；UR-TODO-043-B 依賴已解除；其餘子階段（split allocation、FX attribution、Loan UI／CSV／Import Center consumer mapping 等重大事件）尚未排程或開發）
- 提出日期：2026-07-30
...
- **Remaining Boundaries（FX-A1 Merge 後更新）**：
  ...
  - **Investment buy／sell attribution core**：已完成；不擴大為 foreign investment、realized FX 或其他 consumer。
  - **Loan principal／interest attribution**：**Repository audit confirmed（2026-08-13）**——既有 `loan-payment` atomic contract（L1）已完整覆蓋 identity、atomic group、reconciliation、runtime attribution、void／replacement、persistence，NO-GO DEVELOPMENT，僅補一項 regression test（`tests/loanComponentGroup.test.ts`），零生產程式碼變更；不建立 Generic Split Consumer、不擴大為 foreign loan 或其他 consumer。
...
- **下一正式候選（待盤點，未開始）**：FX-A3 已完成；Loan principal／interest attribution 已於 2026-08-13 完成 Repository audit（NO-GO DEVELOPMENT，既有 contract 已覆蓋）。下一候選依序獨立評估 FX attribution evidence/runtime integration、conversion／realized FX、foreign investment／loan，或 Loan UI／CSV／Import Center producer mapping。**若下一候選涉及 Ledger 寫入語意、schema／persistence 結構性變更、核心 attribution 結果或 AI Decision／Rebalance／Household Liquidity 接線，屬重大產品／核心財務語意事件，須另行拍板，不得自動開始。** UR-TODO-046 整體維持「部分完成／OPEN」，不得因 FX-A1、FX-A2、FX-A3 或本次 Loan audit 完成自行標記整體完成。
...
**UR-TODO-046 Loan principal／interest attribution Repository 唯讀盤點（2026-08-13，NO-GO DEVELOPMENT，Repository audit confirmed）**：先確認既有 `loan-payment` atomic contract（L1，PR #294）是否已完整覆蓋 loan principal／interest attribution，而非預設需要開發。以程式碼實證：principal 恆 0、interest／fee／penalty 恆為一次性負貢獻；`componentId`＋`paymentId`＋`confirmationGroupId` 三層 stable identity；`appendFinancialEventGroup()`／`resolveActiveLoanComponentGroups()` 原子寫入與完整性守恆；reconciliation candidate→matched 升級與三層 double-count 防護皆有程式碼與測試互證；void 一個 component 使整組正確回退、貢獻正確轉移不漏記不重複；persistence round-trip 完整。發現唯一缺口為測試覆蓋（非邏輯缺口）：新增 1 個 regression test（`tests/loanComponentGroup.test.ts`）驗證「兩個完整、皆 posted、皆未 void、共用同一 paymentId」的 duplicate group 會被正確 fail-safe 排除,測試通過且零生產程式碼變更。**結論：不建立第二套 Loan attribution 架構,不改 `netWorthAttribution.ts` 核心公式,不把 Loan 搬到 Generic Split。** `npx tsc -b`、`test:ci`、Production／Preview build 皆成功。UR-TODO-046 下一真正缺口轉向 FX attribution;整體仍 OPEN。
