...
Generated UTC: 2026-08-12T17:37:21.894168+00:00
...
- `003_CURRENT_STATUS.md` — SHA-256 `46d1f669ce656c3dfed4d1db842d8cbcc1a2275b8786901d03b173aceed4b626`
...
- `008_TODO_BACKLOG.md` — SHA-256 `441784751edb6ff06ebb10308167f7e75e8ad776aaf6224e67c7942b15cba64a`
- `009_CHANGELOG.md` — SHA-256 `f0edaab56b2e53fdc589da2c83d0407fddf98bd653ca7ad2a2ac008d322b6309`
...
- `012_AI_HANDOVER.md` — SHA-256 `3ac9a4f325ea824a06688b1c35cd6a227752d35c8759107293ab7b461f3ad55f`
...
- **Remaining Boundaries（FX-A1 Merge 後更新）**：
  ...
  - **FX-A3 Foreign Cash Producer／Snapshot Integration**：開發完成，Draft PR 待 CI／Preview／使用者驗收，尚未 Merge。已修正 mixed-currency naked-sum bug、canonical TWD totals fail-safe（unavailable propagation）、pinned `fxValuations` 實際點亮；未接入 attribution、conversion、realized FX、foreign investment／loan 或其他 consumer，未新增 UI，未修改 Household Liquidity。
...
- **下一正式候選（待盤點，未開始）**：FX-A3 Foreign Cash Producer／Snapshot Integration；其後依序獨立評估 FX attribution evidence/runtime integration、conversion／realized FX、foreign investment／loan，或 Loan UI／CSV／Import Center producer mapping。**若下一候選涉及 Ledger 寫入語意、schema／persistence 結構性變更、核心 attribution 結果或 AI Decision／Rebalance／Household Liquidity 接線，屬重大產品／核心財務語意事件，須另行拍板，不得自動開始。** UR-TODO-046 整體維持「部分完成／OPEN」，不得因 FX-A1 或 FX-A2 完成自行標記整體完成。
...
## 最新交接快照：UR-TODO-046 FX-A3 Foreign Cash Producer / Snapshot Integration（開發完成／Draft PR 待驗收，2026-08-13）

- 基線：branch `feat/ur-todo-046-fx-a3-foreign-cash-snapshot` 從最新 `origin/main`（`b9abbb0ba8bc0195a94ba255a43257689c592ed7`）建立。**Draft PR 尚未建立完成前不得視為正式基線變更**；PR Merge 前 `origin/main` 仍為 `b9abbb0`。
...
- 明確不包含：任何新 UI；FX-A2 startup／render auto-fetch（新增 `tests/fxA3NoAutoFetchRegression.test.ts` 鎖定 `App.tsx` 不 import `cbcFxProvider`）；Household Liquidity（`householdLiquidityInputAdapter.ts` 完全未修改，non-TWD 帳戶維持既有 `unavailable` fail-safe）；FX attribution（`netWorthAttribution.ts`／`runtimeAttributionComposition.ts` 完全未修改——唯讀盤點確認兩者只被動讀取 `snapshot.netWorth`／`.date`，canonical totals 計算方式改變對其型別與邏輯皆無影響）；schema version bump、Backup version bump、migration、historical rewrite 均未發生。
- 驗證：新增 16 個測試（`tests/canonicalNetWorthTotals.test.ts`、`tests/fxA3NoAutoFetchRegression.test.ts` 新檔，`tests/fxValuationPersistence.test.ts` 擴充 2 項）；`npx tsc -b`、`npm run test:ci`、Production／Preview build 皆成功；隔離本機 dev server 以 fixture 實機驗證 mixed-currency（TWD 100,000＋USD 1,000＠31 → 正確 13.1 萬元，非裸加 10.1 萬）與 missing-rate（正確排除 USD、顯示 10 萬元，非裸加或猜值）兩種情境，console 全程無錯誤。
- 下一直接起點：Draft PR 待使用者 Preview 驗收與明確 Merge 指示；**不得自行 Merge、Ready for review 或部署 Production**。UR-TODO-046 整體仍 OPEN；下一個獨立階段（FX attribution evidence/runtime integration、conversion／realized FX、foreign investment／loan 等）均未開始，須另行唯讀盤點與產品決策。
