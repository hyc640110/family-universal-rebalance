...
Generated UTC: 2026-08-18T13:23:54.183770+00:00
...
- `003_CURRENT_STATUS.md` — SHA-256 `0c0fa811445e936338f9cbb7eb6532376694effce023aa63e223046ebc05771d`
...
- `008_TODO_BACKLOG.md` — SHA-256 `dd42e406f4cdf9aaa16a3228f7d3ce94a9c638f8acd769c7edb7ccad4ea8a861`
...
- `012_AI_HANDOVER.md` — SHA-256 `2f749adc8e6c5c45b78b06bb3ddbe445036e61d0c78bd2c332842f755fe2efe6`
...
# Universal Rebalance Current Status v4.29

最後更新：2026-08-18

**UR-TODO-022-C（Batch-Aware Duplicate Reconciliation）已於 2026-08-18 正式 CLOSED／Production Verified；UR-TODO-022 整體仍 PARTIAL／OPEN。** PR [#389](https://github.com/hyc640110/family-universal-rebalance/pull/389) final head `9b2f94b42951aa99665e3e7f90c82d071de16c08` 已於 2026-08-18T13:09:39Z 由 `hyc640110` 以一般 2-parent merge commit `dc6b5aca77c6ad60bb3be243e56091348be0f1ce` 合併（parents：`b720aa3ff15ec367778156528734ef9eca54fadd`／`9b2f94b42951aa99665e3e7f90c82d071de16c08`；未使用 admin override）；現行 `origin/main` 為同一 merge commit。PR CI Verification [run 32139612582](https://github.com/hyc640110/family-universal-rebalance/actions/runs/32139612582)、相同 head 的 Preview workflow_dispatch [run 32139755897](https://github.com/hyc640110/family-universal-rebalance/actions/runs/32139755897)、merge 後 push/main Deploy GitHub Pages [run 32140771082](https://github.com/hyc640110/family-universal-rebalance/actions/runs/32140771082) 均 success；後者為 push/main、head 與 merge commit 一致，regression gate、Production build 與 Pages deploy 均 success。Preview 人工驗收 PASS：同批 exact duplicate 保留最早有效列、後續列為 certain 並預設不選；同日同額同描述但 fingerprint 不同者維持可選並顯示「同批次可能重複，請確認是否仍匯入」；同一非空 externalId 為 certain；category 變更可依既有 batch 規則 downgrade／upgrade，且未 commit 資料。Production 人工唯讀驗收 PASS：`/assets#transactions-section` 的交易基礎、Import Center、檔案選擇與 Column Mapping／Preview 控制項正常，console 無新增 error，未建立或匯入測試交易、未寫入 Production localStorage。Production root 與 Preview root 均 HTTP 200；workflow 來源分別確認為 main Production 與 Preview，Production asset `index-FKnb2K2Y.js`、Preview asset `index-CDobTh5S.js` 分離，兩者皆含 warning 與 external-ID 重複偵測字串。022-C 僅在既有 Import Preview 管線新增 deterministic batch reconciliation：`none`／`possible`／`certain`，possible 保持可選且直接呈現既有 `row.warning`，certain 維持 selected=false；不改 duplicate calculation、category suggestion、checkbox 或資料寫入契約。`test:ci` 1,097 pass（Import Center 36/36）、TypeScript、Production／Preview build、`git diff --check` 均 pass；`npm audit --omit=dev --audit-level=high` 的 4 個 high vulnerabilities 為既有相依問題，非本 Sprint 引入且未處理。無 schema、persistence、localStorage、JSON Backup、Financial Event Ledger、duplicate formula 或 UR-TODO-022-A suggestion contract 變更。依 §8.2 追平基線，PR [#388](https://github.com/hyc640110/family-universal-rebalance/pull/388) 的純治理 merge commit `b720aa3ff15ec367778156528734ef9eca54fadd` 位於 #387 與 #389 之間。下一步改為 **UR-TODO-022 Closeout Scope Review（Review Mode）**，重新檢視 fully automatic classification 是否真為整體結案必要條件；Historical mapping reuse／historical learning／fully automatic classification／AI／LLM classification 維持 deferred，**不得將 UR-TODO-022 整體標記 CLOSED**。

**UR-TODO-022-B（Safe Column Auto-Mapping Hardening）已於 2026-08-18 正式 CLOSED／Production Verified；UR-TODO-022 整體仍 OPEN／PARTIAL。** PR [#387](https://github.com/hyc640110/family-universal-rebalance/pull/387) final head `c9f89aed3232075899799f19fb75889c30828cdc` 已於 2026-08-18T05:52:54Z 由 `hyc640110` 以一般 2-parent merge commit `b81342dac07ace36d965495a74a2a0a628776ff9` 合併（parents：`fd9dafe6044915ac2762ed83979589417a9d7033`／`c9f89aed3232075899799f19fb75889c30828cdc`；未使用 admin override）；現行 `origin/main` 即為同一 merge commit。PR CI Verification [run 32103283273](https://github.com/hyc640110/family-universal-rebalance/actions/runs/32103283273)、相同 head 的 Preview workflow_dispatch [run 32103449472](https://github.com/hyc640110/family-universal-rebalance/actions/runs/32103449472)、merge 後 push/main Deploy GitHub Pages [run 32104607740](https://github.com/hyc640110/family-universal-rebalance/actions/runs/32104607740) 均 success；後者 regression gate、Production build 與 Pages deploy success，head 與 merge commit 一致。Preview 人工驗收 PASS：不手動調整 Column Mapping 即成功產生有效 7／錯誤 0 的 Preview，收入／支出解析與 UR-TODO-022-A category suggestion 正常。Production 人工唯讀驗收 PASS：`/assets#transactions-section` 的交易基礎、Import Center、檔案選擇、Column Mapping、Import Preset／匯入紀錄皆正常，未建立或匯入測試交易、未修改 localStorage 真實資料；Production／Preview HTTP 200、metadata 分別為 `production`／`preview`、assets 隔離，Production bundle 含本 Sprint safe-whitelist aliases。022-B 只將既有 `guessImportMapping` 收緊為 deterministic exact → strong alias：歧義 fail closed、credit＋debit 優先於 amount、交易日期與 posting／入帳日期分離、externalId 嚴格 alias 且裸 `id` 拒絕；duplicate formula 與 022-A preview-only category suggestion 均未變。`test:ci` 1,063 pass、TypeScript、Production／Preview build、`git diff --check` 均 pass；`npm audit --omit=dev --audit-level=high` 的 4 個 high vulnerabilities 為既有相依問題，非本 Sprint 引入且未在本範圍處理。無 schema、persistence、localStorage、JSON Backup、Financial Event Ledger 變更。下一候選僅為 **UR-TODO-022-C Batch Internal Duplicate Detection Contract Audit**；Historical mapping reuse／learning、fully automatic classification、AI／LLM classification 維持 deferred，**不得將 UR-TODO-022 整體標記 CLOSED**。

**UR-TODO-022-A（Rule-Assisted Category Suggestion Foundation）已完成／Production Verified；UR-TODO-022 整體仍 OPEN／PARTIAL。** PR [#385](https://github.com/hyc640110/family-universal-rebalance/pull/385) 的 final head `492b85e99fa72df5389c8a6fe36c37e1990fa18d` 已於 2026-08-18T04:03:57Z 由 `hyc640110` 以一般 2-parent merge commit `9628d8aed9a5875047eb86cdd98b28b7f580849b` 合併（parents：`aba0a9283035ba1e6fd3c55d01a24490aa62943f`／`492b85e99fa72df5389c8a6fe36c37e1990fa18d`；未使用 admin override）；本次 `git fetch --prune origin` 後，現行 `origin/main` 為同一 SHA。PR verify [run 32096747958](https://github.com/hyc640110/family-universal-rebalance/actions/runs/32096747958)、相同 head 的 Preview workflow_dispatch [run 32096761194](https://github.com/hyc640110/family-universal-rebalance/actions/runs/32096761194)、merge 後 push/main Deploy GitHub Pages [run 32097751304](https://github.com/hyc640110/family-universal-rebalance/actions/runs/32097751304) 均 success；後者 regression gate、Production build 與 Pages deploy 均 success，head 與 merge commit 一致。Production root HTTP 200、environment metadata=`production`，實際 bundle 為 `index-UfIXOsXo.js`，未混用 `/preview/` asset，且含本次 Import Preview suggestion contract。使用者已在同一 PR head 的 Preview 完成 Desktop 與 390 × 844 人工驗收；Production 僅做唯讀 deployment verification，未建立測試交易、未假稱重新人工功能驗收。功能維持 deterministic、high-confidence only、safe whitelist only、high-risk semantic fail closed、conflict=`none`；suggestion metadata 僅限 Import Preview/session，須使用者明確套用，無自動交易建立或 commit。未加入 AI／LLM、historical learning、schema／persistence、Ledger／attribution、Household Liquidity 公式或 Rebalance／AI Decision coupling；既有 duplicate detection 不屬本次剩餘範圍。全自動分類仍未實作，因此不得將 UR-TODO-022 標記 CLOSED。
...
- **UR-TODO-022-C = CLOSED／Production Verified；UR-TODO-022 整體仍 PARTIAL／OPEN，絕不可標記 CLOSED。** 下一候選改為 **UR-TODO-022 Closeout Scope Review（Review Mode）**：重新檢視 fully automatic classification 是否真為整體結案必要條件。Historical mapping reuse／historical learning／fully automatic classification／AI／LLM classification 維持 deferred，未經新產品決策與授權不得開始。
