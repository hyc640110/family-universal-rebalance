const VERSION = 'market-data-worker-v6.0.5-refresh';
...
const CBC_USD_TWD_URL = 'https://cpx.cbc.gov.tw/api/OpenData/FTDOpenData_Day';
const SOURCE_TWSE = '臺灣證券交易所 OpenAPI';
const SOURCE_TREASURY = '美國財政部 Daily Treasury Rates';
const SOURCE_CBC_USD_TWD = 'Central Bank of the Republic of China (Taiwan) — Interbank Closing Rate';
...
const cbcDate = value => { const text = String(value ?? '').trim(); if (!/^\d{8}$/.test(text)) return null; const year = Number(text.slice(0, 4)); const month = Number(text.slice(4, 6)); const day = Number(text.slice(6, 8)); const parsed = new Date(Date.UTC(year, month - 1, day)); return parsed.getUTCFullYear() === year && parsed.getUTCMonth() === month - 1 && parsed.getUTCDate() === day ? `${text.slice(0, 4)}-${text.slice(4, 6)}-${text.slice(6, 8)}` : null; };
const fxUnavailable = (reason, capturedAt) => ({ status: 'unavailable', reason, capturedAt });
export function parseCbcUsdTwdLatest(raw, capturedAt) {
  if (!Array.isArray(raw)) return fxUnavailable('schema-changed', capturedAt);
  if (!raw.length) return fxUnavailable('empty-result', capturedAt);
  const byDate = new Map();
  for (const row of raw) {
    if (!row || typeof row !== 'object' || Array.isArray(row) || !Object.hasOwn(row, '日期') || !Object.hasOwn(row, 'NTD_USD')) return fxUnavailable('schema-changed', capturedAt);
    if (typeof row.日期 !== 'string' || typeof row.NTD_USD !== 'string') return fxUnavailable('schema-changed', capturedAt);
    const rateDate = cbcDate(row.日期); const quotePerBase = safeNumber(row.NTD_USD);
    if (!rateDate || quotePerBase === null || quotePerBase <= 0) return fxUnavailable('invalid-latest', capturedAt);
    const previous = byDate.get(rateDate);
    if (previous !== undefined && previous !== quotePerBase) return fxUnavailable('provider-conflict', capturedAt);
    byDate.set(rateDate, quotePerBase);
  }
  const rateDate = [...byDate.keys()].sort().at(-1);
  return { status: 'available', baseCurrency: 'USD', quoteCurrency: 'TWD', rateType: 'reference-close', rateDate, quotePerBase: byDate.get(rateDate), provider: SOURCE_CBC_USD_TWD, capturedAt };
}
async function fetchCbcUsdTwdLatest() { const capturedAt = new Date().toISOString(); const controller = new AbortController(); const timeout = setTimeout(() => controller.abort(), 10_000); try { const response = await fetch(CBC_USD_TWD_URL, { headers: { accept: 'application/json' }, signal: controller.signal }); if (!response.ok) return fxUnavailable('provider-http', capturedAt); try { return parseCbcUsdTwdLatest(await response.json(), capturedAt); } catch { return fxUnavailable('provider-invalid-json', capturedAt); } } catch (error) { return fxUnavailable(error?.name === 'AbortError' ? 'timeout' : 'provider-http', capturedAt); } finally { clearTimeout(timeout); } }
...
const headers = (request, refresh = false) => ({ 'content-type': 'application/json; charset=utf-8', 'access-control-allow-origin': request.headers.get('origin') || '*', 'access-control-allow-methods': 'GET, HEAD, OPTIONS', 'access-control-allow-headers': 'content-type, accept, cache-control', 'vary': 'Origin', 'cache-control': refresh ? 'no-store' : 'public, max-age=300, s-maxage=900' });
const sourceUrl = (base, refresh) => refresh ? `${base}${base.includes('?') ? '&' : '?'}_refresh=${Date.now()}` : base;
async function marketSummary(refresh = false) { const fetchedAt = new Date().toISOString(); const month = fetchedAt.slice(0, 7).replace('-', ''); const [twseResult, treasuryResult] = await Promise.allSettled([fetch(sourceUrl(TWSE_URL, refresh), { headers: { accept: 'application/json', ...(refresh ? { 'cache-control': 'no-cache' } : {}) } }).then(async response => { if (!response.ok) throw new Error(`TWSE HTTP ${response.status}`); return parseTwseIndex(await response.json(), fetchedAt); }), fetch(sourceUrl(`${TREASURY_URL}${month}`, refresh), { headers: { accept: 'application/xml', ...(refresh ? { 'cache-control': 'no-cache' } : {}) } }).then(async response => { if (!response.ok) throw new Error(`Treasury HTTP ${response.status}`); return parseTreasuryLatest(await response.text(), fetchedAt); })]); const items = [twseResult.status === 'fulfilled' ? twseResult.value : unavailable('taiex', 'taiwan', '台灣加權指數', SOURCE_TWSE, '官方資料暫時無法取得。'), ...globalItems(), ...(treasuryResult.status === 'fulfilled' ? treasuryResult.value : parseTreasuryLatest('', fetchedAt)), ...eventItems()]; const failed = [twseResult, treasuryResult].filter(item => item.status === 'rejected').length; return { version: VERSION, fetchedAt, status: failed === 2 ? 'failed' : failed ? 'delayed' : 'recent-effective', items, error: failed ? '部分官方來源暫時無法取得；未以舊值或估算值替代。' : undefined }; }
export default { async fetch(request, env) { const url = new URL(request.url); const refresh = url.searchParams.get('refresh') === '1'; const responseHeaders = headers(request, refresh); if (request.method === 'OPTIONS') return new Response(null, { status: 204, headers: responseHeaders }); if (url.pathname === '/health') return Response.json({ ok: true, version: VERSION, environment: healthEnvironment(env) }, { headers: responseHeaders }); if (url.pathname === '/fx-rates/usd-twd') return Response.json(await fetchCbcUsdTwdLatest(), { headers: responseHeaders }); if (url.pathname !== '/market-summary') return Response.json({ ok: false, error: 'not found' }, { status: 404, headers: responseHeaders }); try { return Response.json(await marketSummary(refresh), { headers: responseHeaders }); } catch { return Response.json({ fetchedAt: new Date().toISOString(), status: 'failed', items: [...globalItems(), ...eventItems()], error: '市場資料服務暫時無法取得。' }, { status: 502, headers: responseHeaders }); } } };
